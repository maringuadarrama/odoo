...
class StockWarehouse(models.Model):
    ...
    pick_type_id = fields.Many2one(
        "stock.picking.type", "Pick Type", check_company=True, copy=False
    )
    pack_type_id = fields.Many2one(
        "stock.picking.type", "Pack Type", check_company=True, copy=False
    )
    out_type_id = fields.Many2one(
        "stock.picking.type", "Out Type", check_company=True, copy=False
    )
    in_type_id = fields.Many2one(
        "stock.picking.type", "In Type", check_company=True, copy=False
    )
    int_type_id = fields.Many2one(
        "stock.picking.type", "Internal Type", check_company=True, copy=False
    )
    qc_type_id = fields.Many2one(
        "stock.picking.type", "Quality Control Type", check_company=True, copy=False
    )
    store_type_id = fields.Many2one(
        "stock.picking.type", "Storage Type", check_company=True, copy=False
    )
    xdock_type_id = fields.Many2one(
        "stock.picking.type", "Cross Dock Type", check_company=True, copy=False
    )
    reception_route_id = fields.Many2one(
        "stock.route", "Receipt Route", ondelete="restrict", copy=False
    )
    delivery_route_id = fields.Many2one(
        "stock.route", "Delivery Route", ondelete="restrict", copy=False
    )
                ...
                if move_ids:
                    raise UserError(
                        _(
                            ...
                            operations=format_list(
                                self.env, move_ids.mapped("picking_type_id.name")
                            ),
                ...
                if picking_type_using_locations:
                    raise UserError(
                        _(
                            ...
                            operations=format_list(
                                self.env, picking_type_using_locations.mapped("name")
                            ),
                            warehouse=warehouse.name,
                        )
                    )
