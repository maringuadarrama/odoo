...
class StockPackage(models.Model):
    ...
    content_description = fields.Char(
        "Contents", compute="_compute_content_description"
    )
    ...
    parent_package_id = fields.Many2one(
        "stock.package", "Container", index="btree_not_null"
    )
    child_package_ids = fields.One2many(
        "stock.package", "parent_package_id", string="Contained Packages"
    )
    ...
    package_dest_id = fields.Many2one(
        "stock.package", "Destination Container", index="btree_not_null"
    )
    ...
    child_package_dest_ids = fields.One2many(
        "stock.package", "package_dest_id", "Assigned Contained Packages"
    )
    ...
    json_popover = fields.Char(
        "JSON data for popover widget", compute="_compute_json_popover"
    )
