...
class Product(models.Model):
    ...
    def write(self, values):
        if 'active' in values:
            self.filtered(
                lambda p: p.active != values['active']
            ).with_context(active_test=False).orderpoint_ids.write({
    ...
    def _compute_quantities(self):
        products = self.with_context(prefetch_fields=False).filtered(
    ...
    def _get_domain_locations(self):
        ...
        if warehouse:
            w_ids = set(
                Warehouse.browse(_search_ids('stock.warehouse', warehouse)).mapped('view_location_id').ids
            )
            if location:
    ...
    def _search_qty_available_new(self, operator, value, lot_id=False, owner_id=False, package_id=False):
        ...
        if not isinstance(value, (float, int)):
            raise UserError(_(
                'Invalid domain right operand \'%s\'. It must be of type Integer/Float',
                value
            ))
    ...
    def _get_rules_from_location(self, location, route_ids=False, seen_rules=False):
        ...
        rule = self.env['procurement.group'].with_context(active_test=True)._get_rule(
            self,
            location,
            {
                'route_ids': route_ids,
