...
class StockLot(models.Model):
    ...
    product_qty = fields.Float(
        "On Hand Quantity", compute="_product_qty", search="_search_product_qty"
    )
    ...
    last_delivery_partner_id = fields.Many2one(
        "res.partner", compute="_compute_last_delivery_partner_id"
    )
    lot_properties = fields.Properties(
        "Properties", definition="product_id.lot_properties_definition", copy=True
    )
    ...
    def _compute_last_delivery_partner_id(self):
        ...
        for lot in serial_products:
            if (
                ...
                and len(delivery_ids_by_lot[lot.id]) > 0
            ):
                lot.last_delivery_partner_id = (
                    self.env["stock.picking"]
                    .browse(delivery_ids_by_lot[lot.id])
                    .sorted(key="date_done", reverse=True)[0]
                    .partner_id
                )
            else:
