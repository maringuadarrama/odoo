...
class PickingType(models.Model):
    ...
    default_location_src_id = fields.Many2one(
        ...
        required=True,
        compute='_compute_default_location_src_id', store=True, precompute=True,
        readonly=False,
        check_company=True,
        help='This is the default source location when this operation is manually created. However, '
             'it is possible to change it afterwards or that the routes use another one by default.',
    )
    default_location_dest_id = fields.Many2one(
        comodel_name='stock.location',
        string='Destination Location',
        required=True,
        compute='_compute_default_location_dest_id', store=True, precompute=True,
        readonly=False,
        check_company=True,
        help='This is the default destination location when this operation is manually created. '
             'However, it is possible to change it afterwards or that the routes use another one by default.'
    )
    ...
    reservation_days_before_priority = fields.Integer(
        'Days when starred',
        help='Maximum number of days before scheduled date that priority picking products '
             'should be reserved.',
    )
    favorite_user_ids = fields.Many2many(
        comodel_name='res.users',
        relation='picking_type_favorite_user_rel',
        column1='picking_type_id', column2='user_id',
    )
    ...
    use_create_lots = fields.Boolean(
        ...
        default=True,
        compute='_compute_use_create_lots', store=True,
        readonly=False,
        help='If this is checked only, it will suppose you want to create new Lots/Serial Numbers, '
             'so you can provide them in a text field.'
    )
    use_existing_lots = fields.Boolean(
        'Use Existing Lots/Serial Numbers',
        default=True,
        compute='_compute_use_existing_lots', store=True,
        readonly=False,
        help='If this is checked, you will be able to choose the Lots/Serial Numbers. You can also '
             'decide to not put lots in this operation type. This means it will create stock with '
             'no lot or not put a restriction on the lot taken.'
    )
    # TODO: delete this field `show_operations`
    show_operations = fields.Boolean(
        'Show Detailed Operations',
        default=False,
        help='If this checkbox is ticked, the pickings lines will represent detailed stock '
             'operations. If not, the picking lines will represent an aggregate of detailed '
             'stock operations.'
    )
    print_label = fields.Boolean(
        'Generate Shipping Labels',
        compute='_compute_print_label', store=True,
        readonly=False,
    ...
    auto_print_delivery_slip = fields.Boolean(
        ...
        help='If this checkbox is ticked, Odoo will automatically print the delivery slip of a '
             'picking when it is validated.'
    )
    auto_print_return_slip = fields.Boolean(
        'Auto Print Return Slip',
        help='If this checkbox is ticked, Odoo will automatically print the return slip of a '
             'picking when it is validated.'
    )
    auto_print_product_labels = fields.Boolean(
        'Auto Print Product Labels',
        help='If this checkbox is ticked, Odoo will automatically print the product labels of a '
             'picking when it is validated.'
    )
    auto_print_lot_labels = fields.Boolean(
        'Auto Print Lot/SN Labels',
        help='If this checkbox is ticked, Odoo will automatically print the lot/SN labels of a '
             'picking when it is validated.'
    )
    auto_show_reception_report = fields.Boolean(
        'Show Reception Report at Validation',
        help='If this checkbox is ticked, Odoo will automatically show the reception report '
             '(if there are moves to allocate to) when validating.'
    )
    auto_print_reception_report = fields.Boolean(
        'Auto Print Reception Report',
        help='If this checkbox is ticked, Odoo will automatically print the reception report of '
             'a picking when it is validated and has assigned moves.'
    )
    auto_print_reception_report_labels = fields.Boolean(
        'Auto Print Reception Report Labels',
        help='If this checkbox is ticked, Odoo will automatically print the reception report '
             'labels of a picking when it is validated.'
    )
    ...
    auto_print_packages = fields.Boolean(
        'Auto Print Packages',
        help='If this checkbox is ticked, Odoo will automatically print the packages and their '
             'contents of a picking when it is validated.'
    )
    auto_print_package_label = fields.Boolean(
        'Auto Print Package Label',
        help='If this checkbox is ticked, Odoo will automatically print the package label when '
             '\'Put in Pack\' button is used.'
    )
            ...
            for picking_type in self:
                if picking_type.warehouse_id:
                    picking_type.sequence_id.sudo().write({
                        ...
                        'prefix': picking_type.warehouse_id.code + '/' + vals['sequence_code'] + '/',
                        'padding': 5,
    ...
    def _onchange_sequence_code(self):
        ...
        if picking_type and picking_type.sequence_id != self.sequence_id:
            return {
                'warning': {
                    'message': _(
                        'This sequence prefix is already being used by another operation type. '
                        'It is recommended that you select a unique prefix to avoid issues and/or '
                        'repeated reference values or assign the existing reference sequence to '
                        'this operation type.'
                    )
                }
            }
    ...
    def _onchange_picking_code(self):
        if self.code == 'internal' and not self.env.user.has_group('stock.group_stock_multi_locations'):
            return {
                'warning': {
                    'message': _(
                        'You need to activate storage locations to be able to do internal '
                        'operation types.'
                    )
                }
            }
