...
class StockWarehouseOrderpoint(models.Model):
    ...
    group_id = fields.Many2one(
        ...
        copy=False,
        help='Moves created through this orderpoint will be put in this procurement group. '
    ...
    product_id = fields.Many2one(
        ...
        check_company=True,
        domain=('''
            [('product_tmpl_id', '=', context.get('active_id', False))] if context.get('active_model') == 'product.template'
            else [('id', '=', context.get('default_product_id', False))] if context.get('default_product_id')
            else [('is_storable', '=', True)]
        '''),
        ondelete='cascade',
    )
    ...
    product_uom_name = fields.Char(
        related='product_uom.display_name', string='Product unit of measure label', readonly=True,
    )
    name = fields.Char(
        string='Name',
        required=True,
    ...
    active = fields.Boolean(
        ...
        help='If the active field is set to False, it will allow '
             'you to hide the orderpoint without removing it.',
    )
    ...
    snoozed_until = fields.Date(
        string='Snoozed',
        help='Hidden until next scheduler.',
    )
    product_min_qty = fields.Float(
        ...
        help='When the virtual stock goes below the Min Quantity specified for this field, '
             'Odoo generates a procurement to bring the forecasted quantity above of '
             'this Min Quantity.',
    )
    product_max_qty = fields.Float(
        ...
        required=True,
        default=0.0,
        compute='_compute_product_max_qty', store=True,
        readonly=False,
        help='When the virtual stock goes below the Min Quantity, Odoo generates '
             'a procurement to bring the forecasted quantity up to (or near to) '
             'the Max Quantity specified for this field (or to Min Quantity, '
             'whichever is bigger).',
    )
    qty_multiple = fields.Float(
        ...
        required=True,
        default=1,
        help='The procurement quantity will be rounded up to a multiple of '
             'this field quantity. If it is 0, it is not rounded.',
    )
    ...
    visibility_days = fields.Float(
        ...
        readonly=False,
        help='Consider product forecast these many days in the future upon product replenishment, '
    ...
    def write(self, vals):
        ...
        if 'snoozed_until' in vals:
            if any(orderpoint.trigger == 'auto' for orderpoint in self):
                raise UserError(_(
                    'You can only snooze manual orderpoints. You should rather archive '
                    '\'auto-trigger\' orderpoints if you do not want them to be triggered.'
                ))
    ...
    @api.depends('product_id.seller_ids', 'product_id.seller_ids.delay', 'rule_ids')
    ...
    def _compute_unwanted_replenish(self):
        for orderpoint in self:
            if (
                ...
                or float_is_zero(
                    orderpoint.qty_to_order, precision_rounding=orderpoint.product_uom.rounding
                )
                or float_compare(
                ...
                after_replenish_qty = orderpoint.product_id.with_context(
                    company_id=orderpoint.company_id.id, location=orderpoint.location_id.id
                ).virtual_available + orderpoint.qty_to_order
                orderpoint.unwanted_replenish = float_compare(
                    after_replenish_qty,
                    orderpoint.product_max_qty,
                    precision_rounding=orderpoint.product_uom.rounding
                ) > 0
