...
class StockWarehouseOrderpoint(models.Model):
    ...
    def _compute_rules(self):
        ...
        product_ids_with_routes = set(
            orderpoints_to_compute.product_id.filter_has_routes().ids
        )
    ...
    def _compute_product_min_qty(self):
        for orderpoint in self:
            if (
                ...
                or not orderpoint.product_min_qty
            ):
        ...
        if force_to_max:
            for orderpoint in self:
                ...
                remainder = (
                    ...
                    and orderpoint.qty_to_order % orderpoint.replenishment_uom_id.factor
                    or 0.0
                )
                if not float_is_zero(
                    remainder, precision_rounding=orderpoint.product_uom.rounding
                ):
                    orderpoint.qty_to_order += (
                        orderpoint.replenishment_uom_id - remainder
                    )
        try:
    ...
    def _get_qty_to_order(
        self, force_visibility_days=False, qty_in_progress_by_orderpoint={}
    ):
        self.ensure_one()
        ...
        if (
            ...
            qty_in_progress = (
                qty_in_progress_by_orderpoint.get(self.id)
                or self._quantity_in_progress()[self.id]
            )
            ...
            qty_multiple = (
                self.replenishment_uom_id._compute_quantity(1, self.product_uom)
            ...
            if (
                ...
                and float_compare(
                    qty_multiple - remainder, 0.0, precision_rounding=rounding
                )
                > 0
            ):
    ...
    def _set_default_route_id(self):
        ...
        for location_dest, route in rules_groups:
            orderpoints = self.filtered(
                lambda o: not o.route_id
                and route in (o.product_id.route_ids | o.product_id.categ_id.route_ids)
                and o.location_id.id == location_dest.id
            )
    ...
    def _get_orderpoint_procurement_date(self):
        return (
            ...
            .localize(datetime.combine(self.lead_days_date, time(12)))
            .astimezone(UTC)
            .replace(tzinfo=None)
        )
