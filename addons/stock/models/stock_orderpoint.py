...
class StockWarehouseOrderpoint(models.Model):
    ...
    product_category_id = fields.Many2one(
        "product.category", name="Product Category", related="product_id.categ_id"
    )
    ...
    allowed_replenishment_uom_ids = fields.Many2many(
        "uom.uom", compute="_compute_allowed_replenishment_uom_ids"
    )
    ...
    qty_on_hand = fields.Float(
        "On Hand", readonly=True, compute="_compute_qty", digits="Product Unit"
    )
    qty_forecast = fields.Float(
        "Forecast", readonly=True, compute="_compute_qty", digits="Product Unit"
    )
    ...
    unwanted_replenish = fields.Boolean(
        "Unwanted Replenish", compute="_compute_unwanted_replenish"
    )
    ...
    def _compute_deadline_date(self):
        ...
        horizon_date = fields.Date.today() + relativedelta.relativedelta(
            days=orderpoints_to_compute.get_horizon_days()
        )
        _, domain_move_in, domain_move_out = (
            orderpoints_to_compute.product_id._get_domain_locations()
        )
        ...
        incoming_moves_by_product_date = Move._read_group(
            domain_move_in, ["product_id", "date:day"], ["product_qty:sum"]
        )
        outgoing_moves_by_product_date = Move._read_group(
            domain_move_out, ["product_id", "date:day"], ["product_qty:sum"]
        )
        ...
        for orderpoint in orderpoints_to_compute:
            ...
            for move_date, move_qty in sorted(
                moves_by_product_dict.get(orderpoint.product_id.id, {}).items()
            ):
            ...
            orderpoint.deadline_date = (
                max(tentative_deadline, fields.Date.today())
                if tentative_deadline < horizon_date
                else False
            )
    ...
    def _compute_lead_days(self):
        ...
        for orderpoint in orderpoints_to_compute.with_context(
            ...
            orderpoint.lead_horizon_date = (
                fields.Date.today()
                + relativedelta.relativedelta(days=lead_days["total_delay"])
            )
            orderpoint.lead_days = (
                lead_days["total_delay"] - orderpoint.get_horizon_days()
            )
    ...
    def get_horizon_days(self):
        ...
        """
        max_company_horizon_days = (
            max(self.company_id.mapped("horizon_days")) if self.company_id else 0
        )
        return self.env.context.get(
            "global_horizon_days",
            max_company_horizon_days or self.env.company.horizon_days,
        )
