...
class StockRule(models.Model):
    ...
    def _push_prepare_move_copy_values(self, move_to_copy, new_date):
        ...
        if (
            float_compare(
                move_to_copy.product_uom_qty,
                0,
                precision_rounding=move_to_copy.product_uom.rounding,
            )
            < 0
        ):
        ...
        new_move_vals = {
            ...
            "description_picking": move_to_copy.product_id.with_context(
                lang=move_to_copy._get_lang()
            )._get_description(self.picking_type_id)
            or move_to_copy.description_picking,
        }
    ...
    def _get_stock_move_values(
        ...
        move_values = {
            ...
            "date": date_scheduled,
            "date_deadline": (
                False if self.group_propagation_option == "fixed" else date_deadline
            ),
            "propagate_cancel": self.propagate_cancel,
    ...
    def _get_lead_days(self, product, **values):
        ...
        if global_visibility_days:
            delay_description.append(
                (_("Time Horizon"), _("+ %d day(s)", int(global_visibility_days)))
            )
...
class ProcurementGroup(models.Model):
    ...
    def run(self, procurements, raise_user_error=True):
        ...
        :raises ProcurementException: if `raise_user_error` is False
        and a procurement isn't fulfillable"""
