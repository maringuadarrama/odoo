...
class StockPackageHistory(models.Model):
    ...
    move_line_ids = fields.One2many(
        "stock.move.line", "package_history_id", string="Move Lines", required=True
    )
    package_id = fields.Many2one("stock.package", "Package", required=True)
    package_name = fields.Char("Package Name", required=True)
    package_type_id = fields.Many2one(
        "stock.package.type", related="package_id.package_type_id"
    )
    ...
    outermost_dest_id = fields.Many2one(
        "stock.package", "Outermost Destination Container"
    )
