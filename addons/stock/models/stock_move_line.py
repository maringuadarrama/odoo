...
class StockMoveLine(models.Model):
    ...
    picked = fields.Boolean(
        "Picked", compute="_compute_picked", store=True, readonly=False, copy=False
    )
    ...
    quant_id = fields.Many2one(
        "stock.quant", "Pick From", store=False
    ...
    def _compute_quantity(self):
        for record in self:
            ...
            if (
                float_compare(
                    move_demand, move_quantity, precision_rounding=sml_uom.rounding
                )
                > 0
            ):
    ...
    def _apply_putaway_strategy(self):
        ...
        for package, smls in groupby(self, lambda sml: sml.result_package_id):
            ...
            if package.package_type_id:
                ...
                smls.location_dest_id = smls.package_level_id.location_dest_id = (
                    best_loc
                )
            elif package:
                used_locations = set()
    ...
    def unlink(self):
        ...
        res = super().unlink()
        package_levels = package_levels.filtered(
            lambda pl: not (pl.move_line_ids or pl.move_ids)
        )
        if package_levels:
    ...
    def _synchronize_quant(
        ...
        available_qty = 0
        if not self.product_id.is_storable or float_is_zero(
            quantity, precision_rounding=self.product_uom_id.rounding
        ):
            return 0, False
        if action == "available":
    ...
    def _free_reservation(
        ...
        for candidate in outdated_candidates:
            ...
            if (
                float_compare(
                    candidate.quantity_product_uom,
                    quantity,
    ...
    def _prepare_stock_move_vals(self):
        ...
        return {
            ...
            )._get_description(self.picking_type_id),
    ...
    def _post_put_in_pack_hook(self, package, **kwargs):
        if package and self.picking_type_id.auto_print_package_label:
            if self.picking_type_id.package_label_to_print == "pdf":
                action = self.env.ref(
                    "stock.action_report_quant_package_barcode_small"
