...
class StockMoveLine(models.Model):
    ...
    state = fields.Selection(
        related='move_id.state', store=True, related_sudo=False
    )
    ...
    quantity_product_uom = fields.Float(
        ...
        compute='_compute_quantity_product_uom', store=True,
        copy=False,
    )
    ...
    consume_line_ids = fields.Many2many(
        ...
        column1='consume_line_id', column2='produce_line_id',
    )
    produce_line_ids = fields.Many2many(
        comodel_name='stock.move.line',
        relation='stock_move_line_consume_rel',
        column1='produce_line_id', column2='consume_line_id',
    )
            ...
            if ml.state == 'done':
                if ml.product_id.is_storable:
                    ...
                    quantity = ml.product_uom_id._compute_quantity(
                        ml.quantity, ml.move_id.product_id.uom_id, rounding_method='HALF-UP'
                    )
                    in_date = None
                    ...
                    if available_qty < 0 and ml.lot_id:
                        ...
                        untracked_qty = Quant._get_available_quantity(
                            ml.product_id, ml.location_id, lot_id=False,
    ...
    def write(self, vals):
        ...
        if ('lot_id' in vals or 'quant_id' in vals) and len(self.product_id) > 1:
            raise UserError(_(
                'Changing the Lot/Serial number for move lines '
                'with different products is not allowed.'
            ))
        ...
        if 'result_package_id' in updates:
            for ml in self.filtered(lambda ml: ml.package_level_id):
                if updates.get('result_package_id'):
                    ...
                    # Only need to unlink the package level if it's empty.
                    # Otherwise will unlink it to still valid move lines.
        ...
        if updates or 'quantity' in vals:
            for ml in self:
                ...
                if not float_is_zero(
                    ml.quantity_product_uom, precision_rounding=ml.product_uom_id.rounding
                ):
        ...
        # When editing a done move line, the reserved availability of a potential chained move
        # is impacted.
        # Take care of running again `_action_assign` on the concerned moves.
        ...
        if 'date' not in vals and ('product_uom_id' in vals or 'quantity' in vals or vals.get('picked', False)):
            ...
            for ml in self:
                ...
                if ('quantity' in vals or 'product_uom_id' in vals) and ml.picked:
                    ...
                    if float_compare(
                        old_qty, new_qty, precision_rounding=ml.product_uom_id.rounding
                    ) < 0:
                        updated_ml_ids.add(ml.id)
        ...
        for ml in mls:
            ...
            ml._synchronize_quant(
                ml.quantity_product_uom, ml.location_dest_id, package=ml.result_package_id
            )
            if available_qty < 0:
        ...
        for line in self:
            ...
            if not line.location_dest_id:
                line.location_dest_id = (
                    line.move_id.location_dest_id
                    or line.picking_id.location_dest_id
                )
    ...
    def _compute_product_packaging_qty(self):
        ...
        for line in self:
            ...
            line.product_packaging_qty = line.move_id.product_packaging_id._compute_qty(
    ...
    def _compute_quantity(self):
        for record in self:
            ...
            move_quantity = record.move_id.product_uom._compute_quantity(
                record.move_id.quantity, sml_uom, rounding_method='HALF-UP'
            )
    ...
    def _onchange_serial_number(self):
        ...
        help him. This includes:
            ...
            - warn (and update if appropriate) if the SN is in a different source location
            than selected
        '''
        res = {}
            ...
            if self.lot_name or self.lot_id:
                ...
                if self.lot_name:
                    ...
                    if counter.get(self.lot_name) and counter[self.lot_name] > 1:
                        message = _(
                            'You cannot use the same serial number twice. '
                        ...
                        if quants:
                            message = _(
                                ...
                                location_list=format_list(
                                    self.env, quants.location_id.mapped('display_name')
                                )
                            )
                elif self.lot_id:
    ...
    @api.onchange('product_id', 'product_uom_id', 'quantity', 'result_package_id')
    ...
    def _synchronize_quant(
        ...
        if (
            ...
            or float_is_zero(quantity, precision_rounding=self.product_uom_id.rounding)
        ):
            return 0, False

        if action == 'available':
            available_qty, in_date = self.env['stock.quant']._update_available_quantity(
                self.product_id, location, quantity, lot_id=lot, package_id=package,
                owner_id=owner, in_date=in_date
            )
        elif action == 'reserved' and not self.move_id._should_bypass_reservation(location):
            self.env['stock.quant']._update_reserved_quantity(
                self.product_id, location, quantity, lot_id=lot,
        ...
        if available_qty < 0 and lot:
            ...
            self.env['stock.quant']._update_available_quantity(
                self.product_id, location, -taken_from_untracked_qty, lot_id=False,
                package_id=package, owner_id=owner, in_date=in_date
            )
            self.env['stock.quant']._update_available_quantity(
                self.product_id, location, taken_from_untracked_qty, lot_id=lot,
                package_id=package, owner_id=owner, in_date=in_date
            )
        return available_qty, in_date
    ...
    def _action_done(self):
        ...
        for ml in self:
            ...
            uom_qty = float_round(
                ml.quantity, precision_rounding=ml.product_uom_id.rounding, rounding_method='HALF-UP'
            )
            precision_digits = self.env['decimal.precision'].precision_get('Product Unit of Measure')
            quantity = float_round(
                ml.quantity, precision_digits=precision_digits, rounding_method='HALF-UP'
            )
            if float_compare(uom_qty, quantity, precision_digits=precision_digits) != 0:
                raise UserError(_(
                    ...
                    'Please change the quantity done or the rounding precision of '
        ...
        if ml_ids_tracked_without_lot:
            ...
            products_list = '\n'.join(
                f'- {product_name}'
                for product_name in mls_tracked_without_lot.mapped('product_id.display_name')
            )
            raise UserError(_(
        ...
        mls_todo.write({'date': fields.Datetime.now()})
    ...
    def _free_reservation(
        ...
        for candidate in outdated_candidates:
            ...
            if float_compare(
                candidate.quantity_product_uom, quantity, precision_rounding=rounding
            ) <= 0:
    ...
    def _log_message(self, record, move, template, vals):
        ...
        if 'lot_id' in vals and vals['lot_id'] != move.lot_id.id:
            data['lot_name'] = self.env['stock.lot'].browse(
                vals.get('lot_id')
            ).name
        ...
        if 'package_result_id' in vals and vals['package_result_id'] != move.package_result_id.id:
            data['result_package_name'] = self.env['stock.quant.package'].browse(
    ...
    def get_move_line_quant_match(self, move_id, dirty_move_line_ids, dirty_quant_ids):
        # Since the quant_id field is neither stored nor computed,
        # this method is used to compute the match if it exists
        ...
        if domain:
            ...
            for quant in quants:
                dirty_lines = dirty_move_lines.filtered(
                    lambda ml:
                        ml.product_id == quant.product_id
                ...
                quants_data.append((
                    ...
                    {
                        'available_quantity':
                            quant.available_quantity + sum(
                                ml.quantity_product_uom
                                for ml in deleted_lines
                            ),
    ...
    def _get_aggregated_product_quantities(self, **kwargs):
        '''
        Returns a dictionary of products (key = id+name+description+uom+packaging) and corresponding 
        values of interest.

        Allows aggregation of data across separate move lines for the same product. This is
        expected to be useful in things such as delivery reports. Dict key is made as a
        combination of values we expect to want to group the products by (i.e. so data is not lost).
        This function purposely ignores lots/SNs because these are expected to already be properly
        grouped by line.

        returns: dictionary {product_id+name+description+uom+packaging:
        {product, name, description, quantity, product_uom, packaging}, ...}
        '''
            ...
            if line_key not in aggregated_move_lines:
                ...
                if backorders and not kwargs.get('strict'):
                    ...
                    following_move_lines = backorders.move_line_ids.filtered(
                        lambda ml:
                            self._get_aggregated_properties(move=ml.move_id)['line_key'] == line_key
                    )
        ...
        for empty_move in pickings.move_ids:
            ...
            if not (
                ...
                and float_is_zero(
                    empty_move.quantity, precision_rounding=empty_move.product_uom.rounding
                )
            ):
                continue
    ...
    def action_put_in_pack(self):
        if len(self.picking_id) > 1:
            raise UserError(_(
                ...
                'package through this view. Try adding them to a batch picking and pack it there.'
            ))
    ...
    def action_revert_inventory(self):
        ...
        for move_line in self:
            if move_line.is_inventory and not float_is_zero(
                move_line.quantity, precision_rounding=move_line.product_uom_id.rounding
            ):
    ...
    def _prepare_stock_move_vals(self):
        ...
        return {
            ...
            'product_id': self.product_id.id,
            'product_uom_qty':
                0 if self.picking_id and self.picking_id.state != 'done'
                else self.quantity,
