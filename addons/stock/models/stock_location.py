...
class Location(models.Model):
    ...
    def default_get(self, fields_list):
        res = super(Location, self).default_get(fields_list)
        if 'barcode' in fields_list and 'barcode' not in res and res.get('complete_name'):
    ...
    replenish_location = fields.Boolean(
        ...
        readonly=False,
        copy=False,
        help='Activate this function to get all quantities to replenish '
    ...
    def _check_scrap_location(self):
        for record in self:
            if record.scrap_location and self.env['stock.picking.type'].search_count(
                ...
                raise ValidationError(_(
                    'You cannot set a location as a scrap location when it is assigned '
                    'as a destination location for a manufacturing type operation.'
                ))
    ...
    def write(self, vals):
        ...
        if 'usage' in vals or 'scrap_location' in vals:
            modified_locations = self.filtered(
                lambda l:
                    any(
                        l[f] != vals[f] if f in vals else False
            ...
            if reserved_quantities:
                raise UserError(_(
                    'Internal locations having stock cannot be converted'
                ))
        ...
        if 'active' in vals:
                ...
                for location in self:
                    ...
                    if warehouses:
                        raise UserError(_(
                            ...
                            'because it is used by warehouse %(warehouse)s',
            ...
            if not self.env.context.get('do_not_check_quant'):
                ...
                if children_quants and not vals['active']:
                    raise UserError(_(
    ...
    def _unlink_except_master_data(self):
        ...
        if inter_company_location in self:
            raise ValidationError(_(
                'The %s location is required by the Inventory app and cannot be deleted, '
                'but you can archive it.',
    ...
    def _compute_next_inventory_date(self):
        for location in self:
            if (
                location.company_id and location.usage in ['internal', 'transit']
    ...
    def _check_can_be_used(self, product, quantity=0, package=None, location_qty=0):
        ...
        if self.storage_category_id:
            ...
            positive_quant = self.quant_ids.filtered(
                lambda q:
                    float_compare(q.quantity, 0, precision_rounding=q.product_id.uom_id.rounding) > 0
            )
