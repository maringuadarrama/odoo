...
class StockPicking(models.Model):
    ...
    package_history_ids = fields.Many2many(
        "stock.package.history", string="Transfered Packages", copy=False
    )
    ...
    def _search_date_category(self, operator, value):
        if operator != "in":
            return NotImplemented
        return Domain.OR(
            self.date_category_to_domain("scheduled_date", item) for item in value
        )
        ...
        for picking in self:
            ...
            for package in relevant_packages:
                if package.shipping_weight:
                    ...
                    shipping_weight += sum(
                        packages_weight[pack]
                        for pack in self.env["stock.package"].browse(
                            children_packages_by_pack[package]
                        )
                    )
