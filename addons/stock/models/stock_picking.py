...
class Picking(models.Model):
    ...
    company_id = fields.Many2one(
        related='picking_type_id.company_id', store=True, string='Company',
        readonly=True,
        index=True,
    )
    ...
    backorder_id = fields.Many2one(
        ...
        copy=False,
        index='btree_not_null',
        help='If this shipment was split, then this field links to the shipment which contains '
             'the already processed part.'
    )
    ...
    return_id = fields.Many2one(
        ...
        copy=False,
        index='btree_not_null',
        help='If this picking was created as a return of another picking, this field links to '
    ...
    move_type = fields.Selection(
        ...
        required=True,
        compute='_compute_move_type', store=True, precompute=True,
        readonly=False,
    ...
    date_deadline = fields.Datetime(
        ...
        help='In case of outgoing flow, validate the transfer before this date to allow '
             'to deliver at promised date to the customer.\n'
             'In case of incoming flow, validate the transfer before this date in order '
             'to have these products in stock at the date promised by the supplier',
    )
    has_deadline_issue = fields.Boolean(
        string='Is late',
        default=False,
        compute='_compute_has_deadline_issue', store=True,
    ...
    group_id = fields.Many2one(
        related='move_ids.group_id', store=True, string='Procurement Group',
        readonly=True,
    )
    product_id = fields.Many2one(
        related='move_ids.product_id', string='Product',
        readonly=True,
    )
    ...
    lot_id = fields.Many2one(
        related='move_line_ids.lot_id', string='Lot/Serial Number', readonly=True,
    )
    ...
    package_level_ids = fields.One2many(
    ...
    has_scrap_move = fields.Boolean(
        string='Has Scrap Moves', compute='_compute_has_scrap_move'
    )
    ...
    show_check_availability = fields.Boolean(
        ...
        help='Technical field used to compute whether the button \'Check Availability\' '
             'should be displayed.',
    )
    show_allocation = fields.Boolean(
        compute='_compute_show_allocation',
        help='Technical Field used to decide whether the button \'Allocation\' '
             'should be displayed.',
    )
    ...
    shipping_weight = fields.Float(
        ...
        help='Total weight of packages and products not in a package. '
             'Packages with no shipping weight specified will default '
             'to their product\'s total weight. '
        ...
        for move in self.env['stock.move'].search([('picking_id', 'in', self.ids)]):
            ...
            picking_moves_state_map[picking_id.id].update({
                ...
                'all_done_are_scrapped':
                    picking_moves_state_map[picking_id.id].get('all_done_are_scrapped', True)
                    and (move.scrapped if move_state == 'done' else True),
                'any_cancel_and_not_scrapped':
                    picking_moves_state_map[picking_id.id].get('any_cancel_and_not_scrapped', False)
                    or (move_state == 'cancel'
    ...
    def _compute_has_deadline_issue(self):
        for picking in self:
            picking.has_deadline_issue = (
                picking.date_deadline and picking.date_deadline < picking.scheduled_date
                or False
            )
    ...
    def _compute_delay_alert_date(self):
        ...
        delay_alert_date_data = {
            picking.id: delay_alert_date
            for picking, delay_alert_date in delay_alert_date_data
        }
        for picking in self:
    ...
    def _compute_products_availability(self):
        ...
        for picking in pickings:
            # In case of draft the behavior of forecast_availability is different:
            # if forecast_availability < 0 then there is a issue else not.
            if any(
                float_compare(
    ...
    @api.depends('state', 'picking_type_code', 'move_ids.product_uom_qty')
    ...
    @api.depends('state', 'picking_type_id', 'move_ids')
    ...
    def _compute_shipping_weight(self):
        for picking in self:
            ...
            picking.shipping_weight = (
                picking.weight_bulk +
                sum(
                    pack.shipping_weight or packages_weight[pack]
                    for pack in picking.move_line_ids.result_package_id
                )
            )
    ...
    def _compute_shipping_volume(self):
        for picking in self:
            volume = 0
            for move in picking.move_ids:
                volume += move.product_uom._compute_quantity(
                ...
                if self.location_id.id not in parent_path:
                    return {
                        'warning': {
                            ...
                            'message': _(
                                'Updating the location of this transfer will result in '
                                'unreservation of the currently assigned items. An attempt to '
                                'reserve items at the new location will be made and the link '
                                'with preceding transfers will be discarded.\n\n'
    ...
    def get_empty_list_help(self, help_message):
        return self.env['ir.ui.view']._render_template(
            ...
            {
                'picking_type_code':
                    self._context.get('restricted_picking_type_code') or self.picking_type_code,
            }
        )
    ...
    def _action_done(self):
        ...
        todo_moves = self.move_ids.filtered(
            lambda self:
                self.state in ['draft', 'waiting', 'partially_available', 'assigned', 'confirmed']
        )
        for picking in self:
            if picking.owner_id:
    ...
    def _check_move_lines_map_quant_package(self, package):
        return package._check_move_lines_map_quant(
            self.move_line_ids.filtered(
                lambda ml:
                    ml.package_id == package and ml.product_id.is_storable
            )
        )
    ...
    def _check_entire_pack(self):
        ...
        for package in self.move_line_ids.package_id:
            ...
            if pickings._check_move_lines_map_quant_package(package):
                package_level_ids = pickings.package_level_ids.filtered(
    ...
    def _get_lot_move_lines_for_sanity_check(self, none_done_picking_ids, separate_pickings=True):
        ...
        :param none_done_picking_ids: Set of all pickings ids that have no quantity set on any
        move_line.
        :param separate_pickings: Indicates if pickings should be checked independently for
        lot/serial numbers or not.
        '''
        ...
        def get_line_with_done_qty_ids(move_lines):
            ...
            return move_lines.filtered(
                lambda ml:
                    ...
                    and ml.picked
                    and float_compare(ml.quantity, 0, precision_rounding=ml.product_uom_id.rounding)
            ).ids
    ...
    def button_validate(self):
        ...
        for move in draft_picking.move_ids:
            if (
                float_is_zero(move.quantity, precision_rounding=move.product_uom.rounding)
    ...
    def should_print_delivery_address(self):
        self.ensure_one()
        return (
            self.move_ids and (self.move_ids[0].partner_id
            or self.partner_id) and self._is_to_external_location()
        )
    ...
    def _package_move_lines(self, batch_pack=False, move_lines_to_pack=False):
        ...
        if len(self.picking_type_id) > 1:
            raise UserError(_(
                'You cannot pack products into the same package when they are from different transfers '
    ...
    def _get_action(self, action_xmlid):
        ...
        action['help'] = self.env['ir.ui.view']._render_template(
            ...
            {
                'picking_type_code':
                    context.get('restricted_picking_type_code')
    ...
    def _get_autoprint_report_actions(self):
        ...
        if self.env.user.has_group('stock.group_production_lot'):
            pickings_print_lot_label = self.filtered(
                lambda p:
                    p.picking_type_id.auto_print_lot_labels
