...
class StockPickingType(models.Model):
    ...
    package_label_to_print = fields.Selection(
        [("pdf", "PDF"), ("zpl", "ZPL")], "Package Label to Print", default="pdf"
    )
...
class StockPicking(models.Model):
    ...
    origin = fields.Char(
        "Source Document", index="trigram", help="Reference of the document"
    )
    ...
    return_count = fields.Integer(
        "# Returns", compute="_compute_return_count", compute_sudo=False
    )
    ...
    warehouse_address_id = fields.Many2one(
        "res.partner", related="picking_type_id.warehouse_id.partner_id"
    )
    ...
    partner_id = fields.Many2one(
        "res.partner", "Contact", check_company=True, index="btree_not_null"
    )
    company_id = fields.Many2one(
        "res.company",
        string="Company",
    ...
    lot_id = fields.Many2one(
        "stock.lot", "Lot/Serial Number", related="move_line_ids.lot_id", readonly=True
    )
    # TODO: delete this field `show_operations`
    ...
    def _compute_location_id(self):
        for picking in self:
            ...
            if picking.picking_type_id:
                ...
                location_dest, location_src = self.env[
                    "stock.warehouse"
    ...
    def _search_products_availability_state(self, operator, value):
        ...
        for picking in self.env["stock.picking"].search(
            ...
            if picking.move_ids._match_searched_availability(
    ...
    def get_empty_list_help(self, help_message):
        return self.env["ir.ui.view"]._render_template(
            "stock.help_message_template",
            {
                "picking_type_code": self._context.get("restricted_picking_type_code")
    ...
    def do_print_picking(self):
        picking_operations_report = self.env.ref(
            "stock.action_report_picking", raise_if_not_found=False
        )
        if not picking_operations_report:
            raise UserError(
                _(
                    "The Picking Operations report has been deleted so you cannot "
                    "print at this time unless the report is restored."
                )
            )
    ...
    def _check_move_lines_map_quant_package(self, package):
        return package._check_move_lines_map_quant(
            self.move_line_ids.filtered(
                lambda ml: ml.package_id == package and ml.product_id.is_storable
            )
        )
        ...
        for package in self.move_line_ids.package_id:
            ...
            if pickings._check_move_lines_map_quant_package(package):
                ...
                if not package_level_ids:
                    if len(pickings) == 1:
                        package_location = (
                            pickings._get_entire_pack_location_dest(move_lines_to_pack)
                            or pickings.location_dest_id.id
                        )
                    ...
                    for pl in package_level_ids:
                        pl.location_dest_id = (
                            pickings._get_entire_pack_location_dest(pl.move_line_ids)
                            or pickings.location_dest_id.id
                        )
        ...
        if not self._should_show_transfers():
            ...
            if pickings_without_lots:
                message += _(
                    ...
                    transfer_list=format_list(
                        self.env, pickings_without_lots.mapped("name")
                    ),
                    product_list=format_list(
                        self.env, products_without_lots.mapped("display_name")
                    ),
                )
            if message:
    ...
    def action_split_transfer(self):
        ...
        if all(
            float_compare(
                m.quantity, m.product_uom_qty, precision_rounding=m.product_uom.rounding
            )
            == 0
            for m in self.move_ids
        ):
