...
class StockQuant(models.Model):
    ...
    inventory_diff_quantity = fields.Float(
        string='Difference',
        digits='Product Unit of Measure',
        compute='_compute_inventory_diff_quantity', store=True,
        readonly=True,
        help='Indicates the gap between the product\'s theoretical quantity '
             'and its counted quantity.',
    )
    inventory_date = fields.Date(
        string='Scheduled Date',
        compute='_compute_inventory_date', store=True,
        readonly=False,
    ...
    inventory_quantity_set = fields.Boolean(
        default=False,
        compute='_compute_inventory_quantity_set', store=True,
        readonly=False,
    )
        ...
        for quant in self:
            if quant.location_id.usage == 'view':
                raise ValidationError(_(
                    'You cannot take products from or deliver products to '
                    'a location of type \'view\' (%s).',
    ...
    def _unlink_except_wrong_permission(self):
        if not self.env.is_superuser():
            if not self.env.user.has_group('stock.group_stock_manager'):
                raise UserError(_(
                    'Quants are auto-deleted when appropriate. '
                    'If you must manually delete them, please ask a stock manager to do it.'
                ))
    ...
    def _compute_last_count_date(self):
        ...
        groups = self.env['stock.move.line']._read_group(
            ...
            [
                'product_id', 'lot_id', 'package_id', 'owner_id', 'result_package_id',
    ...
    def _onchange_location_or_product_id(self):
        ...
        if self.product_id and self.location_id:
            ...
            self.quantity = sum(quant.filtered(
                lambda q: q.lot_id == self.lot_id).mapped('quantity')
            )
    ...
    def _onchange_serial_number(self):
        if self.lot_id and self.product_id.tracking == 'serial':
            message, dummy = self.env['stock.quant'].sudo()._check_serial_number(
                self.product_id,
                self.lot_id,
                self.company_id
            )
    ...
    def _get_gather_domain(
        ...
        if self.env.context.get('with_expiration'):
            domain = expression.AND(
                [['|', ('expiration_date', '>=', self.env.context['with_expiration']), ('expiration_date', '=', False)], domain]
            )
        return domain

    def _gather(
        self, product_id, location_id, lot_id=None, package_id=None, owner_id=None, strict=False, qty=0
    ):
        '''
    ...
    def _get_available_quantity(
        ...
        if product_id.tracking == 'none':
            ...
            if allow_negative:
                ...
                return sum(
                    [
                        available_quantity for available_quantity in availaible_quantities.values()
                        if float_compare(available_quantity, 0, precision_rounding=rounding) > 0
                    ]
                )

    def _get_reserve_quantity(
        self, product_id, location_id, quantity, product_packaging_id=None,
        uom_id=None, lot_id=None, package_id=None, owner_id=None, strict=False
    ):
        '''
        ...
        rounding = product_id.uom_id.rounding
        quants = self._gather(
            product_id, location_id, lot_id=lot_id, package_id=package_id, owner_id=owner_id, strict=strict, qty=quantity
        )
        ...
        if float_compare(quantity, 0, precision_rounding=rounding) > 0:
            ...
            available_quantity = sum(
                ...
                ).mapped('quantity')) - sum(quants.mapped('reserved_quantity')
            )
    ...
    def _update_available_quantity(
        self, product_id, location_id, quantity=False, reserved_quantity=False,
        ...
        if location_id.should_bypass_reservation():
            ...
            incoming_dates = [
                ...
                if quant.in_date and float_compare(
                    quant.quantity,
                    0,
                    precision_rounding=quant.product_uom_id.rounding
                ) > 0
        ...
        if quants:
            ...
            self._cr.execute(
                ...
                WHERE
                    id IN %s
                ORDER BY
                    lot_id LIMIT 1 FOR NO KEY UPDATE SKIP LOCKED
                ''',
                [tuple(quants.ids)]
            )
    ...
    def _update_reserved_quantity(
        self, product_id, location_id, quantity, lot_id=None, package_id=None, owner_id=None, strict=True
    ):
        '''
        ...
        self._update_available_quantity(
            product_id, location_id, reserved_quantity=quantity, lot_id=lot_id,
        ...
        if product_id.tracking == 'serial':
            ...
            if lot_id.company_id:
                internal_domain = expression.AND(
                    [internal_domain, [('company_id', '=', company_id.id)]]
                )
            ...
            if quants:
                if not source_location_id:
                    ...
                    message = _(
                        ...
                        'validated before its corresponding receipt operation is validated. '
                        'In this case the issue will be solved automatically once all steps are '
                    ...
                    if recommended_location and recommended_location.company_id == company_id:
                        message = _(
                            'Serial number (%(serial_number)s) is not located in '
                            '%(source_location)s, but is located in location(s): '
                            '%(other_locations)s.\n\n'
                            'Source location for this move will be changed to '
                            '%(recommended_location)s',
                            serial_number=lot_id.name,
                            source_location=source_location_id.display_name,
                            other_locations=format_list(
                                self.env, sn_locations.mapped('display_name')
                            ),
    ...
    def get_aggregate_barcodes(self):
        ...
        barcode_separator = self.env['ir.config_parameter'].sudo().get_param(
