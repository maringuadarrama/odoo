...
class ProductProduct(models.Model):
    ...
    def fields_get(self, allfields=None, attributes=None):
        ...
        context_location = self._context.get("location") or self._context.get(
            "search_location"
        )
    ...
    def action_view_orderpoints(self):
        ...
        if self and len(self) == 1:
            ...
            action["domain"] = expression.AND(
                [action.get("domain") or [], [("product_id", "in", self.ids)]]
            )
        return action
    ...
    def _get_dates_info(self, date, location, route_ids=False):
        ...
        return {
            ...
            "date_order": date
            - relativedelta(
                days=delays["security_lead_days"] + delays["purchase_delay"]
            ),
        }
    ...
    def write(self, values):
        ...
        if any(key in values for key in keys_to_protect):
            changed = self.filtered(
                lambda u: any(
                    f in values and u[f] != values[f]
                    for f in {"factor", "relative_factor"}
                )
                or "relative_uom_id" in values
                and u.relative_uom_id.id != int(values["relative_uom_id"])
            )
            if changed:
                error_msg = _(
