...
class QuantPackage(models.Model):
    ...
    location_id = fields.Many2one(
        'stock.location',
        'Location',
        compute='_compute_package_info', store=True,
        readonly=False,
        index=True,
    )
    ...
    package_use = fields.Selection(
        ...
        required=True,
        default='disposable',
        help='''Reusable boxes are used for batch picking and emptied afterwards to be reused.
             In the barcode application, scanning a reusable box will add the products in this box.
             Disposable boxes aren't reused, when scanning a disposable box in the barcode application,
             the contained products are added to the transfer.
             ''',
    )
