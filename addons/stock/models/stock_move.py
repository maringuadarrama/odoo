...
class StockMove(models.Model):
    ...
    # used to depict a restriction on the ownership of quants
    ...
    location_dest_id = fields.Many2one(
        ...
        readonly=False,
        index=True,
        compute='_compute_location_dest_id', store=True, precompute=True,
    ...
    scrapped = fields.Boolean(
        related='location_dest_id.scrap_location', store=True,
        string='Scrapped',
        readonly=True
    )
    route_ids = fields.Many2many(
        ...
        column1='move_id', column2='route_id',
    ...
    move_dest_ids = fields.Many2many(
        ...
        column1='move_orig_id', column2='move_dest_id',
    ...
    move_orig_ids = fields.Many2many(
        comodel_name='stock.move',
        relation='stock_move_move_rel',
        column1='move_dest_id', column2='move_orig_id',
    ...
    date_deadline = fields.Datetime(
        ...
        readonly=True,
        copy=False,
        help='In case of outgoing flow, validate the transfer before this date to allow to deliver '
             'at promised date to the customer.\n'
             'In case of incoming flow, validate the transfer before this date in order to have '
             'these products in stock at the date promised by the supplier'
    )
    reservation_date = fields.Date(
        string='Date to Reserve',
        compute='_compute_reservation_date', store=True,
    ...
    product_qty = fields.Float(
        ...
        digits=0,
        compute='_compute_product_qty', store=True, compute_sudo=True,
    ...
    picked = fields.Boolean(
        ...
        readonly=False,
        copy=False,
        help='This checkbox is just indicative, it doesn\'t validate '
             'or generate any product moves.',
    )
    ...
    is_locked = fields.Boolean(
        compute='_compute_is_locked', readonly=True
    )
    ...
    def init(self):
        ...
        if not self._cr.fetchone():
            self._cr.execute(
                'CREATE INDEX stock_move_product_location_index ON stock_move '
    ...
    @api.depends('state', 'date', 'move_orig_ids.date', 'move_orig_ids.state')
    ...
    @api.depends('state', 'date', 'picking_type_id', 'priority')
    def _compute_reservation_date(self):
        for move in self:
            if (
                move.picking_type_id.reservation_method == 'by_date'
                and move.state in ['draft', 'confirmed', 'waiting', 'partially_available']
            ):
    ...
    def _compute_location_dest_id(self):
        for move in self:
            ...
            if (
                ...
                and (move.location_final_id._child_of(location_dest)
                ...
                # - The location dest is an out location (i.e. Customers) but the final dest is
                # different (e.g. Inter-Company transfers)
                location_dest = move.location_final_id
            move.location_dest_id = location_dest

    # inverse method for location_dest_id
    def _set_location_dest_id(self):
        for ml in self.move_line_ids:
            parent_path = [
                int(loc_id) for loc_id in ml.location_dest_id.parent_path.split('/')[:-1]
            ]
    ...
    @api.depends(
        'state',
        'picking_type_id.use_create_lots', 'picking_type_id.use_existing_lots',
        'picking_code',
        'origin_returned_move_id',
        'product_id.type', 'has_tracking',
    )
    ...
    @api.depends('state', 'product_id', 'product_uom', 'product_uom_qty')
    ...
    def _compute_product_packaging_qty(self):
        ...
        for move in self:
            ...
            move.product_packaging_qty = move.product_packaging_id._compute_qty(
                move.product_qty
            )

    @api.depends('state', 'location_id', 'product_id', 'product_qty')
    def _compute_product_availability(self):
        ...
        for move in self:
            if move.state == 'done':
                ...
                total_availability = self.env['stock.quant']._get_available_quantity(
                    move.product_id, move.location_id
                ) if move.product_id else 0.0
        ...
        for move in self:
            ...
            if float_compare(uom_qty, qty, precision_digits=precision_digits) != 0:
                err.append(_(
                    ...
                    precision defined on the unit of measure %(unit)s. Please change the quantity 
                    done or the rounding precision of your unit of measure.
                    ''',
                    product=move.product_id.display_name,
                    unit=move.product_uom.display_name
                ))
                continue
    ...
    def _compute_product_packaging_quantity(self):
        ...
        for move in self:
            ...
            move.product_packaging_quantity = move.product_packaging_id._compute_qty(
    ...
    @api.depends(
        'state', 'picking_type_id', 'location_id', 'priority',
        'product_id', 'product_qty', 'quantity', 'product_uom_qty'
    )
    def _compute_forecast_information(self):
        ...
        for key_context, product_ids in prefetch_virtual_available.items():
            read_res = self.env['product.product'].browse(product_ids).with_context(
                warehouse_id=key_context[0], to_date=key_context[1]
            ).read(['virtual_available'])
            virtual_available_dict[key_context] = {
                res['id']: res['virtual_available']
                for res in read_res
            }
        ...
        for move in product_moves:
            if move._is_consuming():
                ...
                forecast_availability = (
                    virtual_available_dict[key_virtual_available(move, incoming=True)][move.product_id.id]
                )
                if move.state == 'draft':
    ...
    def _onchange_suggest_packaging(self):
        ...
        if self.product_id and self.product_qty and self.product_uom:
            self.product_packaging_id = self.product_id.packaging_ids._find_suitable_product_packaging(
        ...
        if self.product_uom.factor > self.product_id.uom_id.factor:
            return {
                'warning': {
                    ...
                    'message': _(
                        'You are using a unit of measure smaller than the one you are '
                        'using in order to stock your product. This can lead to rounding '
                        'problem on reserved quantity. You should use the smaller unit '
                        'of measure possible in order to valuate your stock or change '
                        'its rounding precision to a smaller value (example: 0.00001).'
                    ),
                }
            }
    ...
    def action_open_reference(self):
        '''
        Open the form view of the move's reference document, if one exists,
        otherwise open form view of self
        '''
        self.ensure_one()
    ...
    def _propagate_product_packaging(self, product_package_id):
        ...
        '''
        already_propagated_ids = (
            self.env.context.get('product_packaging_propagation_ids', set()) | set(self.ids)
        )
    ...
    def _set_date_deadline(self, new_deadline):
        ...
        # (up and down stream - only update by up/downstream documents)
    ...
    def _get_relevant_state_among_moves(self):
        ...
        moves_todo = self.filtered(
            ...
            key=lambda move:
                (sort_map.get(move.state, 0), move.product_uom_qty)
        )
        if not moves_todo:
            return 'assigned'
    ...
    def _merge_moves_fields(self):
        '''
        This method will return a dict of stock move’s values that represent the values
        of all moves in `self` merged.
        '''
    ...
    def _merge_moves(self, merge_into=False):
        ...
        if moves_to_unlink:
            # We are using propagate to False in order to not cancel
            # destination moves merged in moves[0]
    ...
    def _prepare_procurement_qty(self):
        ...
        for move in self:
            if move.id not in mtso_moves or float_compare(
                move.product_qty,
                0,
    ...
    def _prepare_procurement_values(self):
        ...
        comming from a stock move. This method could be override in order to add other custom key
        that could be used in move/po creation.
        '''
        self.ensure_one()
        ...
        if (
            ...
            and self.location_id.warehouse_id.lot_stock_id.parent_path in self.location_id.parent_path
        ):
        ...
        return {
            'product_description_variants':
                self.description_picking and self.description_picking.replace(
    ...
    def _push_apply(self):
        ...
        for move in self:
            ...
            # if the move is a returned move, we don't want to check push rules, as returning a
            # returned move is the only decent way to receive goods without triggering the push
            # rules again (which would duplicate chained operations) first priority goes to the
            # preferred routes defined on the move itself (e.g. coming from a SO line)
    ...
    def _action_confirm(self, merge=True, merge_into=False):
        ...
        # procure_method sometimes changes with certain workflows so just in case,
        # apply to all moves
        ...
        # + those expected to be auto-assigned
        moves.filtered(
            lambda move:
    ...
    def _get_available_move_lines_out(self, assigned_moves_ids, partially_available_moves_ids):
        move_lines_out_done = (self.move_orig_ids.mapped('move_dest_ids') - self).filtered(
        ...
        for k, g in groupby(move_lines_out_reserved, key=_keys_out_groupby):
            grouped_move_lines_out[k] = sum(
                self.env['stock.move.line'].concat(*list(g)).mapped('quantity_product_uom')
            )
    ...
    def _get_available_move_lines(self, assigned_moves_ids, partially_available_moves_ids):
        ...
        return dict(
            (k, v) for k, v in available_move_lines.items()
    ...
    def _update_reserved_quantity(
        ...
        quants = self.env['stock.quant']._get_reserve_quantity(
            self.product_id, location_id, need,
            product_packaging_id=self.product_packaging_id,
            uom_id=self.product_uom, lot_id=lot_id, package_id=package_id,
        ...
        for reserved_quant, quantity in grouped_quants.values():
            ...
            if to_update and float_compare(
            ...
            if move._should_bypass_reservation():
                ...
                if (
                    ...
                    to_update = move.move_line_ids.filtered(
                        lambda ml:
                            ml.product_uom_id == move.product_uom
    ...
    def _recompute_state(self):
        ...
        for move in self:
            ...
            if move.state in ('cancel', 'done') or (move.state == 'draft' and not move.quantity):
            ...
            elif move.quantity and float_compare(
                move.quantity, move.product_uom_qty, precision_rounding=rounding
            ) <= 0:
                ...
                # In the process of merging a negative move, we may still have a 
                # negative move in the move_orig_ids at that point.
    ...
    def _create_backorder(self):
        ...
        for move in self:
            # To know whether we need to create a backorder or not, round to the general
        ...
        # The backorder moves are not yet in their own picking. We do not want to check entire
        # packs for those ones as it could messed up the result_package_id of the moves being
        # currently validated
    ...
    def _skip_push(self):
        return (
            ...
            or self.location_final_id
                and self.location_final_id._child_of(self.location_dest_id)
        )
    ...
    def _action_done(self, cancel_backorder=False):
        ...
        moves = (
            self | moves).exists().filtered(lambda x: x.state not in ('done', 'cancel')
        )
        ...
        for move in moves:
            if move.picked:
                # in theory, we should only have a mix of picked and non-picked mls
                # in the barcode use case where non-scanned mls = not picked =>
                # we definitely don't want to validate them
        ...
        for result_package in moves_todo.move_line_ids.filtered(
            lambda ml: ml.picked
        ).mapped('result_package_id').filtered(
        ...
        if any(
            ml.package_id
            and ml.package_id == ml.result_package_id
    ...
    def _set_product_qty(self):
        '''
        The meaning of product_qty field changed lately and is now a functional field computing the 
        quantity in the default product UoM. This code has been added to raise an error if a write 
        is made given a value for `product_qty`, where the same write should set the 
        `product_uom_qty` field instead, in order to detect errors.
        '''
        raise UserError(_(
    ...
    def _split(self, qty, restrict_partner_id=False):
        ...
        :param restrict_partner_id: optional partner that can be given in order to force 
        the new move to restrict its choice of quants to the ones belonging to this partner.
        ...
        if self.state in ('done', 'cancel'):
            ...
            # we restrict the split of a draft move because if not confirmed yet,
            # it may be replaced by several other moves in case of phantom bom
            # (with mrp module). And we don't want to deal with this complexity by
    ...
    def _get_available_quantity(
        self, location_id, lot_id=None, package_id=None, owner_id=None, strict=False, allow_negative=False
    ):
        self.ensure_one()
    ...
    def _get_forecast_availability_outgoing(self, warehouse, location_id=False):
        ...
        :return: a defaultdict of outgoing moves from warehouse for product_id in self,
        values are tuple (sum_qty_expected, max_date_expected)
        :rtype: defaultdict
        '''
        ...
        def get_stock_moves(moves, state):
            if state == 'available':
                ...
                return moves.filtered(
                    lambda m:
                        m.forecast_availability == m.product_qty
                        and m.forecast_expected_date
                        and m.forecast_expected_date <= get_comparison_date(m)
                )
            elif state == 'late':
                return moves.filtered(
                    lambda m:
                ...
                return moves if moves.filtered(
    ...
    def _delay_alert_get_documents(self):
        ...
        to post the delay alert next activity. These documents are deduplicated. This method
        is meant to be overridden by other modules, each of them adding an element by type of
        recordset on this list.
    ...
    def _convert_string_into_field_data(self, string, options):
        ...
        # Number => Quantity.
        if regex_findall(r'^([0-9]+\.?[0-9]*|\.[0-9]+)$', string):
    ...
    # renamed function from action_action_generate_lot_line_vals
    @api.model
