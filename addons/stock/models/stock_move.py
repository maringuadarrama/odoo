...
class StockMove(models.Model):
    ...
    picking_id = fields.Many2one(
        "stock.picking", "Transfer", index=True, check_company=True
    )
    ...
    package_ids = fields.One2many(
        "stock.package", string="Packages", compute="_compute_package_ids"
    )
    ...
    restrict_partner_id = fields.Many2one(
        "res.partner", "Owner ", check_company=True, index="btree_not_null"
    )
...
Please change the quantity done or the rounding precision in your settings.""",
    ...
    def _compute_forecast_information(self):
        ...
        for warehouse, moves_ids in outgoing_unreserved_moves_per_warehouse.items():
            ...
            for location, mvs in moves_per_location.items():
                ...
                for move in mvs:
                    move.forecast_availability, move.forecast_expected_date = (
                        forecast_info[move]
                    )
    ...
    def _generate_serial_numbers(
        ...
        if self.picking_type_id.use_existing_lots or self.env.context.get(
            "force_lot_m2o"
        ):
    ...
    def _should_assign_at_confirm(self):
        return (
            ...
            or (self.reservation_date and self.reservation_date <= fields.Date.today())
        )
    ...
    def _action_done(self, cancel_backorder=False):
        ...
        moves_todo.write(
            {
                "state": "done",
                "date": self.env.context.get("counting_date") or fields.Datetime.now(),
            }
        )
