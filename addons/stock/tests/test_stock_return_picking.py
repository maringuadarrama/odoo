...
class TestReturnPicking(TestStockCommon):
    ...
    def test_return_picking_SN_pack(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            product_serial, wh_stock, 1.0, lot_id=serial1
        )
        ...
        customer_stock = self.env["stock.quant"]._gather(
            product_serial, customer_location, lot_id=serial1
        )
    ...
    def test_stock_picking_report_has_return(self):
        ...
        serial_1 = self.env["stock.lot"].create(
            {"name": "SN1", "product_id": product_serial.id}
        )
        serial_2 = self.env["stock.lot"].create(
            {"name": "SN2", "product_id": product_serial.id}
        )

        self.env["stock.quant"]._update_available_quantity(
            product_serial, wh_stock, 1.0, lot_id=serial_1
        )
        self.env["stock.quant"]._update_available_quantity(
            product_serial, wh_stock, 1.0, lot_id=serial_2
        )
