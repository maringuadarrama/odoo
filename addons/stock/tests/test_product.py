...
class TestVirtualAvailable(TestStockCommon):
    ...
    def test_product_template_multiwarehouse_quantities(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product_3, warehouse_one.lot_stock_id, 200
        )
        self.env["stock.quant"]._update_available_quantity(
            self.product_3, warehouse_two.lot_stock_id, 50
        )

        product_template = self.product_3.product_tmpl_id
        main_qty = product_template.with_context(
            warehouse_id=warehouse_one.id
        ).qty_available
        other_qty = product_template.with_context(
