...
class TestSinglePicking(TestStockCommon):
    ...
    def test_picking_reservation_at_confirm(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            product, self.env["stock.location"].browse(self.stock_location), 5
        )
        self.env["procurement.group"].run_scheduler()
        self.assertRecordValues(
        ...
        self.env["stock.quant"]._update_available_quantity(
            product, self.env["stock.location"].browse(self.stock_location), 10
        )
        self.env["procurement.group"].run_scheduler()
        self.assertRecordValues(
...
class TestRoutes(TestStockCommon):
    ...
    def test_location_dest_update(self):
        ...
        product = self.env["product.product"].create(
            {"name": "new_product", "is_storable": True, "route_ids": [(4, route.id)]}
        )
        move1 = self.env["stock.move"].create(
            {
                "name": "move with a route",
