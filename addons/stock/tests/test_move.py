...
class StockMove(TransactionCase):
    ...
    def test_past_quantity(self):
        ...
        product_in_past = self.product.with_context(
    ...
    def test_product_tree_views(self):
        ...
        template_view = Form(
            ...
            .with_user(user)
            .browse(self.product.product_tmpl_id.id),
    ...
    def test_scrap_7_sn_warning(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product_serial, child_loc1, 1, lot_id=lot1
        )
    ...
    def test_scrap_11(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product, scrap_location, 10
        )
        picking = self.env["stock.picking"].create(
            {
                "name": "A single picking with one move to scrap",
        ...
        quant = self.env["stock.quant"]._gather(
            self.product, self.stock_location, strict=True
        )
    ...
    def test_move_line_aggregated_product_quantities(self):
        ...
        aggregate_val_3 = aggregate_values[
            f"{product4.id}_{product4.name}__{sml3.product_uom_id.id}"
        ]
        ...
        aggregate_val_4 = aggregate_values[
            f"{product4.id}_{product4.name}__{sml4.product_uom_id.id}"
        ]
    ...
    def test_skip_putaway_if_dest_loc_set_by_user(self):
        ...
        with Form(
            receipt.move_ids, view="stock.view_stock_move_operations"
        ) as move_form:
            with move_form.move_line_ids.edit(0) as line:
    ...
    def test_free_reservation(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product, self.stock_location, 5
        )
    ...
    def test_autocomplete_sml_location_based_on_sm_lot_ids(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product_serial, subloc, 1, lot_id=lots[1], package_id=pack
        )
    ...
    def test_change_dest_loc_after_sm_creation(self):
        ...
        self.assertEqual(
            form.move_ids_without_package.edit(0).location_dest_id,
            self.customer_location,
        )

        picking = form.save()
        self.assertEqual(
            picking.move_ids_without_package.location_dest_id, self.customer_location
        )
        ...
        self.assertEqual(
            form.move_ids_without_package.edit(0).location_dest_id, self.stock_location
        )
        self.assertEqual(
            form.move_ids_without_package.edit(1).location_dest_id, self.stock_location
        )

        picking = form.save()
        self.assertEqual(
            picking.move_ids_without_package.location_dest_id, self.stock_location
        )
