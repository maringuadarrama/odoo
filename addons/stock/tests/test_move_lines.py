...
class StockMoveLine(TestStockCommon):
    ...
    def test_pick_from_1(self):
        ...
        self.assertEqual(
            self.quant.display_name, "WH/Stock/Shelf 1 - Lot 1 - Pack A - The Owner"
        )
    ...
    def test_multi_edit_quant_and_lot(self):
        ...
        """
        self.env["stock.quant"]._update_available_quantity(
            self.product, self.shelf1, 20, lot_id=self.lot, owner_id=self.partner
        )
        quant_productA = self.env["stock.quant"]._update_available_quantity(
            self.productA, self.shelf1, 20, owner_id=self.partner
        )
        picking1 = self.env["stock.picking"].create(
            {
                "name": "Picking 1",
