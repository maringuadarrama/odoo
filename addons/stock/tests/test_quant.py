...
class StockQuant(TransactionCase):
    ...
    def test_closest_removal_strategy_tracked(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product_serial, self.stock_subloc3, 1.0, lot_id=lot1, in_date=in_date
        )
        # Add a product from lot2 in stock_location/subloc2
        self.env["stock.quant"]._update_available_quantity(
            self.product_serial, self.stock_subloc2, 1.0, lot_id=lot2, in_date=in_date
        )
    ...
    def test_update_quant_with_forbidden_field(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            product, self.stock_subloc2, 1.0, lot_id=sn1
        )
        self.assertEqual(len(product.stock_quant_ids), 1)
        self.env["stock.quant"]._update_available_quantity(
            product, self.stock_subloc3, 1.0, lot_id=sn1
        )
        ...
        with self.assertRaises(UserError):
            quant_2.with_context(inventory_mode=True).write(
                {"location_id": self.stock_subloc2}
            )
    ...
    def test_relocate(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product, self.stock_location, 10, package_id=package_01
        )
        ...
        quants_ab = self.env["stock.quant"].search(
            [
                ("product_id", "in", (self.product.id, product_b.id)),
                ("quantity", "=", 10),
            ]
        )
        ...
        new_quants_abc = self.env["stock.quant"].search(
            [
                ("product_id", "in", (self.product.id, product_b.id, product_c.id)),
                ("quantity", "=", 10),
            ],
            order="product_id",
        )
    ...
    def test_inventory_adjustment_package(self):
        ...
        dummy_package = self.env["stock.quant.package"].create(
            {"name": "dummy package"}
        )
