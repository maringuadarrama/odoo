...
class TestReports(TestReportsCommon):
    ...
    def test_report_reception_2_two_receipts(self):
        ...
        report.action_unassign(
            [mto_move.id], receipt2_qty, receipt2.move_ids_without_package.ids
        )
        mto_move = delivery.move_ids_without_package.filtered(
            lambda m: m.procure_method == "make_to_order"
        )
        ...
        delivery2.action_confirm()
        self.assertEqual(
            delivery2.move_ids_without_package.quantity,
            outgoing_qty,
            "Delivery move should already be reserved",
        )
        report.action_assign(
            delivery2.move_ids_without_package.ids,
            [incoming_qty],
            (receipt1 | receipt2).move_ids_without_package.ids,
        )
        mto_move = delivery2.move_ids_without_package.filtered(
            lambda m: m.procure_method == "make_to_order"
        )
    ...
    def test_report_reception_7_done_receipt(self):
        ...
        self.assertEqual(len(delivery.move_ids_without_package), 1)
        self.assertEqual(
            delivery.move_ids_without_package.quantity,
            outgoing_qty,
            "Delivery move should already be reserved",
        )
        report = self.env["report.stock.report_reception"]
        ...
        self.assertEqual(
            len(delivery.move_ids_without_package.move_orig_ids),
            1,
            "Only 1 delivery + 1 receipt move should be assigned",
        )
        self.assertEqual(
            len(receipt.move_ids_without_package.move_dest_ids),
            1,
            "Receipt move should remain unsplit",
        )
