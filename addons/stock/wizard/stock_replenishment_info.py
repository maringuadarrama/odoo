...
class StockReplenishmentInfo(models.TransientModel):
            ...
            replenishment_report.json_lead_days = dumps(
                {
                    ...
                    "visibility_days_date": format_date(
                        self.env,
                        replenishment_report.orderpoint_id.lead_days_date
                        + relativedelta(days=orderpoint.visibility_days),
                    ),
                }
            )
    ...
    def _compute_json_replenishment_history(self):
        for replenishment_report in self:
            ...
            quantity_by_month_returned = dict(
                ...
                replenishment_history.append(
                    {
                        "name": babel.dates.format_datetime(
                            month, format=fmt, locale=locale
                        ),
                        "quantity": product_qty_sum
