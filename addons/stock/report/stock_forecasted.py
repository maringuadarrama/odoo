...
class StockForecasted_Product_Product(models.AbstractModel):
    ...
    def _get_report_header(self, product_template_ids, product_ids, wh_location_ids):
        ...
        [in_sum] = self.env["stock.move"]._read_group(
            in_domain, aggregates=["product_qty:sum"]
        )[0]
        [out_sum] = self.env["stock.move"]._read_group(
            out_domain, aggregates=["product_qty:sum"]
        )[0]
    ...
    def _get_report_lines(
        ...
        def _get_out_move_taken_from_stock_data(
            ...
            for move in linked_moves:
                ...
                if float_is_zero(
                    demand, precision_rounding=move.product_id.uom_id.rounding
                ):
                    continue
        ...
        past_outs = self.env["stock.move"].search(
            AND([out_domain, past_domain]), order="priority desc, date, id"
        )
        future_outs = self.env["stock.move"].search(
            AND([out_domain, future_domain]),
        ...
        qties = self.env["stock.quant"]._read_group(
            [
                ("location_id", "in", wh_location_ids),
                ("quantity", ">", 0),
                ("product_id", "in", outs.product_id.ids),
            ],
        ...
        for product in (ins | outs).product_id:
            ...
            if not float_is_zero(free_stock, precision_rounding=product_rounding):
                lines.append(
                    self._prepare_report_line(free_stock, product=product, read=read)
                )
            # In moves not used.
    ...
    def action_unreserve_linked_picks(self, move_id):
        ...
        if move_ids:
            ...
            move_ids.picking_id.package_level_ids.filtered(
                lambda p: not p.move_ids
            ).unlink()
        return move_ids
