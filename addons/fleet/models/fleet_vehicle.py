...
class FleetVehicle(models.Model):
            ...
            for vehicle in self.filtered(lambda v: v.driver_id.id != driver_id):
                ...
                if vehicle.driver_id:
                    vehicle.activity_schedule(
                        ...
                        note=_(f'Specify the End date of {vehicle.driver_id.name}')
                    )
    ...
    def _compute_service_activity(self):
        for vehicle in self:
            activities_state = set(
                state for state in vehicle.service_ids.mapped('activity_state')
    ...
    @api.depends('service_ids', 'service_ids.odometer')
    def _compute_odometer(self):
        for vehicle in self:
            if vehicle.service_ids:
                vehicle.odometer = max(vehicle.service_ids.mapped('odometer'))
            else:
    ...
    def _compute_count_all(self):
        ...
        contract_data = LogContract._read_group(
            [('vehicle_id', 'in', self.ids), ('state', '!=', 'closed')], ['vehicle_id', 'active'], ['__count']
        )
