        ...
        self.env.cr.execute(
            ...
            CREATE OR replace VIEW vendor_delay_report AS(
                SELECT m.id                  AS id,
                    ...
                    SUM(
                        CASE WHEN (m.state = 'done' and pol.date_planned::date >= m.date::date)
                            THEN ((ml.quantity * ml_uom.factor) / pt_uom.factor)
                            ELSE 0
                        END
                    )                        AS qty_on_time
                FROM
