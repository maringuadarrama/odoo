...
class PurchaseOrder(models.Model):
    ...
    # Extend the field to make it computed
    ...
    def action_cancel(self):
        ...
        purchase_orders_with_receipt = self.filtered(
            lambda o: any(move.state == "done" for move in o.order_line_ids.move_ids)
        )
        if purchase_orders_with_receipt:
            raise UserError(
                _(
                    "Unable to cancel purchase order(s): %s since "
                    "they have receipts that are already done.",
                    format_list(
                        self.env, purchase_orders_with_receipt.mapped("display_name")
                    ),
                )
            )
        ...
        for order in self:
            ...
            pickings_to_cancel_ids.update(
                order.picking_ids.filtered(lambda r: r.state != "cancel").ids
            )
        ...
        for order_line in order_lines:
            ...
            if order_line.procurement_group_id:
                order_line.procurement_group_id.purchase_line_ids = [Command.unlink(order_line.id)]
    ...
    def _get_action_view_picking(self, pickings):
        """This function returns an action that display existing picking orders of given
        purchase order ids. When only one found, show the picking immediately."""
        self.ensure_one()
    ...
    def _prepare_picking(self):
        if not self.procurement_group_id:
            self.procurement_group_id = self.procurement_group_id.create(self._prepare_group_vals())
    ...
    def _prepare_create_invoice_vals(self):
        invoice_vals = super()._prepare_create_invoice_vals()
    ...
    def prepare_dashboard(self):
        ...
        purchases = self.env["purchase.order"].search_fetch(
            ...
            ["date_planned", "date_effective", "purchase_user_id"],
        )
    ...
    def _create_picking(self):
        ...
        for order in self.filtered(lambda po: po.state == "purchase"):
            if any(
                product.type == "consu" for product in order.order_line_ids.product_id
            ):
    ...
    def _add_picking_info(self, activity):
        ...
        if validated_picking:
            message = _(
                "Those dates couldn’t be modified accordingly on the receipt %s "
                "which had already been validated.",
