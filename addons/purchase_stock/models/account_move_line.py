...
class AccountMoveLine(models.Model):
    ...
    def _apply_price_difference(self):
        ...
        for line in self:
            ...
            po_line = line.purchase_line_ids
            uom = line.product_uom_id or line.product_id.uom_id
            # Don't create value for more quantity than received
            quantity = po_line.qty_transfered - (po_line.qty_invoiced - line.quantity)
            ...
            layers = line._get_valued_in_moves().stock_valuation_layer_ids.filtered(
                lambda svl: svl.product_id == line.product_id
    ...
    def _generate_price_difference_vals(self, layers):
        ...
        """
        self.ensure_one()
        po_line = self.purchase_line_ids
        ...
        for aml in po_line.invoice_line_ids:
            move = aml.move_id
        ...
        qty_to_invoice_per_layer, layers_and_invoices_qties = self._replay_history(
            layers, history
        )
        ...
        for layer in layers:
            ...
            if float_is_zero(
                invoicing_layer_qty, precision_rounding=product_uom.rounding
            ):
                continue
            ...
            if self.is_refund:
                ...
                initial_invoiced_qty = layers_and_invoices_qties[
                ...
                if not initial_pdiff_svl or float_is_zero(
                    initial_invoiced_qty, precision_rounding=product_uom.rounding
                ):
                    continue
                ...
                for item in history:
                    ...
                    previously_invoiced_qty += layers_and_invoices_qties[
                        (
                            layer,
                            previous_invoice,
                        )
                ...
                remaining_out_qty_to_invoice = max(
                    0, out_layer_qty - invoiced_layer_qty
                )
                out_qty_to_invoice = min(
                    remaining_out_qty_to_invoice, invoicing_layer_qty
                )
            ...
            svl_vals, aml_vals = self._prepare_pdiff_vals(
                layer, aml, layer_price_unit, out_qty_to_invoice, sign * qty_to_correct
            )
    ...
    def _replay_history(self, layers, history):
        ...
        for _time, aml, layer in history:
            if layer:
                ...
                initial_layer = initial_layer.filtered(
                ...
                if initial_layer:
                    ...
                    initial_layer_remaining_qty = qty_to_invoice_per_layer[
                        initial_layer
                    ][1]
                    common_qty = min(
                        initial_layer_remaining_qty, total_layer_qty_to_invoice
                    )
                    qty_to_invoice_per_layer[initial_layer][0] -= common_qty
                    qty_to_invoice_per_layer[initial_layer][1] -= common_qty
                    total_layer_qty_to_invoice = max(
                        0, total_layer_qty_to_invoice - common_qty
                    )
                if (
                    float_compare(
                        total_layer_qty_to_invoice,
                ...
                aml_qty = aml.product_uom_id._compute_quantity(
                ...
                while (
                    float_compare(
                        aml_qty, 0, precision_rounding=self.product_id.uom_id.rounding
                    )
                    > 0
    ...
    def _get_valued_in_moves(self):
        self.ensure_one()
        return self.purchase_line_ids.move_ids.filtered(
    ...
    def _get_out_and_not_invoiced_qty(self, in_moves):
        ...
        invoiced_qty = sum(
            ...
            for line in self.purchase_line_ids.invoice_line_ids - self
        )
    ...
    def _get_price_unit_val_dif_and_relevant_qty(self):
        ...
        if self.product_id.cost_method != "standard" and self.purchase_line_ids:
            ...
            price_unit = (
                -price_unit if self.move_id.move_type == "in_refund" else price_unit
            )
            valuation_date = (
                valuation_stock_moves
        ...
        price_unit = self._get_price_unit_gross()
    ...
    def _prepare_pdiff_aml_vals(self, qty, unit_valuation_difference):
        ...
        expense_account = self.product_id.product_tmpl_id.get_product_accounts(
            ...
            vals_list.append(
                {
                    ...
                    or self.move_id.commercial_partner_id.id,
                    "currency_id": self.currency_id.id,
                    "product_id": self.product_id.id,
                    "product_uom_id": self.product_uom_id.id,
                    "balance": self.company_id.currency_id.round(
                        (qty * price * sign) / self.currency_rate
                    ),
    ...
    def _prepare_pdiff_svl_vals(self, corrected_layer, quantity, unit_cost, pdiff):
        ...
        common_svl_vals = {
            ...
            and f"{self.move_id.name} - {self.product_id.name}"
            or self.product_id.name,
        }
        return {
            **self.product_id._prepare_in_svl_vals(
                quantity, unit_cost, corrected_layer.lot_id
            ),
            **common_svl_vals,
    ...
    def _prepare_pdiff_vals(
        ...
        aml_gross_price_unit = aml._get_price_unit_gross()
        # convert from aml currency to company currency
        aml_price_unit = aml_gross_price_unit / aml.currency_rate
        aml_price_unit = aml.product_uom_id._compute_price(
            aml_price_unit, self.product_id.uom_id
        )
        ...
        unit_valuation_difference_curr = self.product_id.uom_id._compute_price(
            unit_valuation_difference_curr, self.product_uom_id
        )
        out_qty_to_invoice = self.product_id.uom_id._compute_quantity(
        ...
        po_pu_curr = self.purchase_line_ids.currency_id._convert(
            self.purchase_line_ids.price_unit,
            self.currency_id,
            self.company_id,
        ...
        if not float_is_zero(
            ...
            svl_vals = self._prepare_pdiff_svl_vals(
                layer, qty_to_correct, unit_valuation_difference, price_difference_curr
            )
