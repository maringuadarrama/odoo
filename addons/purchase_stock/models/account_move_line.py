...
class AccountMoveLine(models.Model):
    ...
    def _generate_price_difference_vals(self, layers):
        """The method will determine which layers are impacted by the AML
        (`self`) and in case of a price difference, it will then return
        the values of the new AMLs and SVLs"""
        self.ensure_one()
        ...
        for _time, aml, layer in history:
            if layer:
                ...
                while (
                    float_compare(aml_qty, 0, precision_rounding=product_uom.rounding)
                    > 0
                    ...
                    layers_and_invoices_qties[(layer, invoice)] = [
                        common_qty,
                        common_qty,
                    ]
                    layers_and_invoices_qties[(layer, impacted_invoice)][
                        1
                    ] -= common_qty
        ...
        for layer in layers:
            ...
            unit_valuation_difference_curr = (
                unit_valuation_difference * self.currency_rate
            )
    ...
    def _get_price_unit_val_dif_and_relevant_qty(self):
        ...
        valuation_stock_moves = (
            self.env["stock.move"].search(
                [
                    ("purchase_line_id", "=", self.purchase_line_ids.id),
                    ("state", "=", "done"),
                    ("product_qty", "!=", 0.0),
                ]
            )
