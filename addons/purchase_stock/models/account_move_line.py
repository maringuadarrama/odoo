...
class AccountMoveLine(models.Model):
    ...
    def _generate_price_difference_vals(self, layers):
        """The method will determine which layers are impacted by the AML (`self`) and, in case
        of a price difference, it will then return the values of the new AMLs and SVLs"""
        self.ensure_one()
    ...
    def _apply_price_difference(self):
        ...
        for line in self:
            ...
            layers = line._get_valued_in_moves().stock_valuation_layer_ids.filtered(
                lambda svl:
                    svl.product_id == line.product_id
        ...
        return self.env["stock.valuation.layer"].sudo().create(svl_vals_list),\
            self.env["account.move.line"].sudo().create(aml_vals_list)
