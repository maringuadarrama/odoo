...
class AccountMove(models.Model):
    ...
    def _post(self, soft=True):
        ...
        for product, company in unique(
            (svl.product_id, svl.company_id) for svl in stock_valuation_layers
        ):
        ...
        for lot, company in unique(
            (svl.lot_id, svl.company_id) for svl in stock_valuation_layers
        ):
            if not lot:
                continue
            ...
            if not float_is_zero(
                lot.quantity_svl, precision_rounding=lot.product_id.uom_id.rounding
            ):
        ...
        posted = super(
            AccountMove, self.with_context(skip_cogs_reconciliation=True)
        )._post(soft)
    ...
    def _stock_account_get_last_step_stock_moves(self):
        ...
        for invoice in self.filtered(lambda x: x.move_type in ("in_invoice", "in_refund")):
    ...
    def _stock_account_prepare_anglo_saxon_in_lines_vals(self):
        """Prepare values used to create the journal items (account.move.line) corresponding to the
        price difference lines for vendor bills. It only concerns the quantities that have been
        delivered before the bill
        Example:
        Buy a product having a cost of 9 and a supplier price of 10 and being a storable product
        and having a perpetual valuation in FIFO. Deliver the product and then post the bill.
        The vendor bill's journal entries looks like:
        ...
        to env["account.move.line"].create."""
        lines_vals_list = []
