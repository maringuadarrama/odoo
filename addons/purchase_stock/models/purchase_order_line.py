...
class PurchaseOrderLine(models.Model):
    ...
    def _create_activity_smaller_than_qty_invoiced(self):
    ...
    def _create_stock_moves(self, picking):
        values = []
        for line in self.filtered(lambda l: not l.display_type):
            for val in line._prepare_stock_move_vals_list():
    ...
    def _hook_on_created_confirmed_lines(self):
        ...
        for line in self:
            line._update_or_create_picking()
    ...
    def _hook_on_written_confirmed_lines(self, write_vals, previous_vals):
        ...
        for line in self:
            ...
            if (
                ...
                line._create_activity_smaller_than_qty_invoiced()
                line.with_context(
                    previous_product_qty=previous_vals[line.id].get(
                        "product_uom_qty", {}
                    )
                )._update_or_create_picking()

    def _add_up_pol_vals(self, rfq_line):
        super()._add_up_pol_vals(rfq_line)
        self.move_dest_ids += rfq_line.move_dest_ids

    def _update_date_planned(self, new_date):
        # Only change the date if there are no moves or any move is done
        if not self.move_ids or not any(move.state == "done" for move in self.move_ids):
            super()._update_date_planned(new_date)

        if any(move.state not in ("done", "cancel") for move in self.move_ids):
            self._update_stock_move_date_deadline(new_date)
    ...
    def _update_or_create_picking(self):
        ...
        for line in self:
            if (
                ...
                or not line.product_uom_qty > line.qty_transfered
            ):
                continue
    ...
    def _find_candidate(
        ...
        """Return the record in self where the procument with
        values passed as args can be merged. If it returns an
        empty record then a new line will be created."""
        description_picking = ""
    ...
    def _prepare_aml_vals(self, **optional_values):
        ...
        if "balance" not in res:
            res["balance"] = self.currency_id._convert(
                self.price_unit_discounted_taxexc * (self.qty_received or 1),
