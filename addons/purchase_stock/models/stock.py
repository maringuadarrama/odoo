...
class StockPicking(models.Model):
    ...
    days_to_arrive = fields.Datetime(
        compute="_compute_effective_date", search="_search_days_to_arrive", copy=False
    )
...
class StockWarehouseOrderpoint(models.Model):
    ...
    show_supplier = fields.Boolean(
        "Show supplier column", compute="_compute_show_supplier"
    )
    ...
    def _get_replenishment_multiple_alternative(self, qty_to_order):
        ...
        if global_visibility_days:
            planned_date -= relativedelta.relativedelta(
