...
class ProductSupplierinfo(models.Model):
    ...
    def _compute_last_purchase_date(self):
        ...
        purchases = self.env["purchase.order"].search(
            [
                ...
                ("order_line_ids.product_id", "in", self.product_tmpl_id.product_variant_ids.ids),
    ...
    def _compute_display_name(self):
        for supplier in self:
            if (
                self.env.context.get("display_supplier_uom_qty")
                and (
                    supplier.min_qty > 1
    ...
    def action_set_supplier(self):
        ...
        if "buy" not in orderpoint.route_id.rule_ids.mapped("action"):
            orderpoint.route_id = self.env["stock.rule"].search(
                [("action", "=", "buy")], limit=1
            ).route_id.id
