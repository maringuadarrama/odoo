...
class StockMove(models.Model):
    ...
    def _get_price_unit(self):
        ...
        order = line.order_id
        received_qty = line.qty_transfered
        if self.state == "done":
        ...
        if (
            ...
            if (
                ...
                price_unit = line._get_price_unit_gross()
        else:
            price_unit = line._get_price_unit_gross()
        ...
        if order.currency_id != order.company_id.currency_id:
            ...
            if (
                ...
                convert_date = max(
                    line.sudo()
                    .invoice_line_ids.move_id.filtered(lambda m: m.state == "posted")
                    .mapped("invoice_date"),
    ...
    def _get_upstream_documents_and_responsibles(self, visited):
        ...
        if created_pl:
            return [(pl.order_id, pl.order_id.purchase_user_id, visited) for pl in created_pl]

        elif self.purchase_line_id and self.purchase_line_id.state != "cancel":
            return [
                (
                    self.purchase_line_id.order_id,
                    self.purchase_line_id.order_id.purchase_user_id,
                    visited,
                )
            ]
