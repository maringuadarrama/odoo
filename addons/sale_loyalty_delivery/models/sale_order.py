...
class SaleOrder(models.Model):
    ...
    def _get_no_effect_on_threshold_lines(self):
        ...
        return res + self.order_line_ids.filtered(
    ...
    def _get_not_rewarded_order_lines(self):
        ...
        order_lines = super()._get_not_rewarded_order_lines()
        return order_lines.filtered(lambda line: not line.is_delivery)

    def _get_reward_values_free_shipping(self, reward, coupon, **kwargs):
        delivery_line = self.order_line_ids.filtered(lambda l: l.is_delivery)[:1]
        ...
        return [{
            ...
            'order_id': self.id,
            'is_reward_line': True,
            'sequence': max(self.order_line_ids.filtered(lambda x: not x.is_reward_line).mapped('sequence'), default=0) + 1,
    ...
    def _get_claimable_rewards(self, forced_coupons=None):
        ...
        if any(reward.reward_type == 'shipping' for reward in self.order_line_ids.reward_id):
