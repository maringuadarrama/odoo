    ...
    def _get_invoiced_lot_values(self):
        ...
        for sml in stock_move_lines:
            ...
            if (
                ...
                or product_uom.compare(previous_qty_delivered, previous_qty_invoiced)
                < 0
            ):
                previously_done = (
                    quantity
                    if is_stock_return
                    else min(previous_qty_invoiced - previous_qty_delivered, quantity)
                )
    ...
    def _compute_delivery_date(self):
        ...
        for move in self:
            sale_order_effective_date = list(
                filter(
                    None, move.line_ids.sale_line_ids.order_id.mapped("effective_date")
                )
            )
            effective_date_res = (
                max(sale_order_effective_date) if sale_order_effective_date else False
            )
