...
class StockMove(models.Model):
    ...
    def action_generate_lot_line_vals(
        ...
        if product.use_expiration_date:
            ...
            expiration_date = from_date + datetime.timedelta(
                days=product.expiration_time
            )
    ...
    def _get_formating_options(self, strings):
        ...
        for string in strings:
            ...
            if int(value_1) > 31:
                ...
                break
            elif int(value_1) > 12 and (
                re_findall("[a-zA-Z]", value_2) or int(value_2) <= 12
            ):
                ...
                if re_findall(
                    "^%[mbB]", user_lang_format
                ):  # First parameter is for month.
                    return options
                elif re_findall(
                    "^%[djaA]", user_lang_format
                ):  # First parameter is for day.
                    options["dayfirst"] = True
                    break
                elif re_findall(
                    "^%[yY]", user_lang_format
                ):  # First parameter is for year.
                    options["yearfirst"] = True
                    break
        return options
