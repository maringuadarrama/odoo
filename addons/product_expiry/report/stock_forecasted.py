...
class StockForecasted_Product_Product(models.AbstractModel):
    ...
    def _get_report_header(self, product_template_ids, product_ids, wh_location_ids):
        ...
        res["use_expiration_date"] = any(
            self.env["product.product"]
            .browse(res["product_variants_ids"])
            .mapped("use_expiration_date")
        )
    ...
    def _get_products_to_always_include(self, products, product_templates):
        if not products:
            return (
                ...
                .browse(product_templates)
                .product_variant_ids.filtered(lambda p: p.use_expiration_date)
            )
        return (
            self.env["product.product"]
            .browse(products)
            .filtered(lambda p: p.use_expiration_date)
        )

    def _free_stock_lines(self, product, free_stock, moves_data, wh_location_ids, read):
        res = []
        if product.use_expiration_date:
            reserved_expired, unreserved_expired = self.env["stock.quant"]._read_group(
                self._get_expired_quant_domain(wh_location_ids, product),
                aggregates=["reserved_quantity:sum", "available_quantity:sum"],
            )[0]
            ...
            for removal_date, free_stock_at_date in self.env["stock.quant"]._read_group(
                ...
                if (
                    not float_is_zero(
                        free_stock_at_date, precision_rounding=product.uom_id.rounding
                    )
                    and removal_date
                ):
                    res.append(
