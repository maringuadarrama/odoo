...
class SaleOrder(models.Model):
    ...
    @api.depends('order_line_ids', 'amount_total', 'order_line_ids.invoice_line_ids.parent_state', 'order_line_ids.invoice_line_ids.price_total', 'order_line_ids.pos_order_line_ids')
    def _compute_amount_unpaid(self):
        for sale_order in self:
            total_invoice_paid = sum(sale_order.order_line_ids.filtered(lambda l: not l.display_type).mapped('invoice_line_ids').filtered(lambda l: l.parent_state != 'cancel').mapped('price_total'))
