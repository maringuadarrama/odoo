...
class ReportMrpReport_Bom_Structure(models.AbstractModel):
    ...
    def _get_bom_data(self, bom, warehouse, product=False, line_qty=False, bom_line=False, level=0, parent_bom=False, parent_product=False, index=0, product_info=False, ignore_stock=False, simulated_leaves_per_workcenter=False):
        ...
        bom_report_line = {
            ...
            'link_id': (product.id if product.count_product_variant > 1 else product.product_tmpl_id.id) or bom.product_tmpl_id.id,
            'link_model': 'product.product' if product.count_product_variant > 1 else 'product.template',
    ...
    def _get_component_data(self, parent_bom, parent_product, warehouse, bom_line, line_quantity, level, index, product_info, ignore_stock=False):
        ...
        return {
            ...
            'link_id': bom_line.product_id.id if bom_line.product_id.count_product_variant > 1 else bom_line.product_id.product_tmpl_id.id,
            'link_model': 'product.product' if bom_line.product_id.count_product_variant > 1 else 'product.template',
    ...
    def _get_byproducts_lines(self, product, bom, bom_quantity, level, total, index):
        ...
        for byproduct in bom.byproduct_ids:
            ...
            byproducts.append({
                ...
                'type': 'byproduct',
                'link_id': byproduct.product_id.id if byproduct.product_id.count_product_variant > 1 else byproduct.product_id.product_tmpl_id.id,
                'link_model': 'product.product' if byproduct.product_id.count_product_variant > 1 else 'product.template',
                'currency_id': company.currency_id.id,
