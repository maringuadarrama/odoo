...
class SaleOrder(models.Model):
    ...
    def write(self, vals):
        ...
        if any(line.service_tracking == 'event' for line in self.order_line_ids) and vals.get('partner_id'):
    ...
    def action_confirm(self):
        unconfirmed_registrations = self.order_line_ids.registration_ids.filtered(
        ...
        for so in self:
            if not any(line.service_tracking == 'event' for line in so.order_line_ids):
                continue
            so_lines_missing_events = so.order_line_ids.filtered(lambda line: line.service_tracking == 'event' and not line.event_id)
            ...
            so.order_line_ids._init_registrations()
            if len(self) == 1:
