...
class ProjectShareWizard(models.TransientModel):
    ...
    share_link = fields.Char(
        "Public Link", help="Anyone with this link can access the project in read mode."
    )
    collaborator_ids = fields.One2many(
        "project.share.collaborator.wizard", "parent_wizard_id", string="Collaborators"
    )
    ...
    def action_share_record(self):
        ...
        return {
            ...
            "view_mode": "form",
            "views": [
                (self.env.ref("project.project_share_wizard_confirm_form").id, "form")
            ],
