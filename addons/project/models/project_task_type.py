...
class ProjectTaskType(models.Model):
    ...
    disabled_rating_warning = fields.Text(
        compute="_compute_disabled_rating_warning",
        export_string_translation=False,
    )
    fold = fields.Boolean(string="Folded in Kanban")
    auto_validation_state = fields.Boolean(
    ...
    def copy_data(self, default=None):
        ...
        return [
            dict(vals, name=self.env._("%s (copy)", task_type.name))
            for task_type, vals in zip(self, vals_list)
        ]
    ...
    def _unlink_if_remaining_personal_stages(self):
        ...
        remaining_personal_stages_all = self.env["project.task.type"]._read_group(
            [
                ("user_id", "in", personal_stages.user_id.ids),
                ("id", "not in", personal_stages.ids),
            ],
        ...
        for user, sequence, stage in remaining_personal_stages_all:
            remaining_personal_stages_by_user[user].append(
                {"id": stage.id, "seq": sequence}
            )
        ...
        for user in personal_stages.user_id:
            if not user.active or user.share:
                continue
            user_stages_to_unlink = personal_stages.filtered(
                lambda stage: stage.user_id == user
            )
            user_remaining_stages = remaining_personal_stages_by_user[user]
            if not user_remaining_stages:
                raise UserError(
                    _(
                        "Each user should have at least one personal stage. Create a new stage to which the tasks can be transferred after the selected ones are deleted."
                    )
                )
            user_stages_to_unlink._prepare_personal_stages_deletion(
                user_remaining_stages, personal_stages_to_update
            )
    ...
    def unlink_wizard(self, stage_view=False):
        ...
        readgroup = (
            ...
            .env["project.task"]
            ._read_group([("stage_id", "in", self.ids)], ["project_id"])
        )
        project_ids = list(
            set([project.id for [project] in readgroup] + self.project_ids.ids)
        )
    ...
    def action_unarchive(self):
        ...
        if stage_active and self.env["project.task"].with_context(
            active_test=False
        ).search_count(
    ...
    def _prepare_personal_stages_deletion(
        self, remaining_stages_dict, personal_stages_to_update
    ):
        ...
        """
        stages_to_delete_dict = sorted(
            [{"id": stage.id, "seq": stage.sequence} for stage in self],
        ...
        for stage in stages_to_delete_dict:
            while (
                next_replacement_stage and next_replacement_stage["seq"] < stage["seq"]
            ):
                replacement_stage_id = next_replacement_stage["id"]
                next_replacement_stage = (
                    remaining_stages_dict and remaining_stages_dict.pop()
                )
