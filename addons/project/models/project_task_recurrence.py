...
class ProjectTaskRecurrence(models.Model):
    ...
    def _create_next_occurrences(self, occurrences_from):
        ...
        occurrences_from = occurrences_from.filtered(
            lambda task: task.recurrence_id.repeat_type != "until"
            or not task.date_deadline
            or task.recurrence_id.repeat_until
            and (task.date_deadline + task.recurrence_id._get_recurrence_delta()).date()
            <= task.recurrence_id.repeat_until
        )
        if occurrences_from:
    ...
    def _create_next_occurrences_values(self, recurrence_by_task):
        ...
        for task, copy_data, fields_to_copy, fields_to_postpone in zip(
            tasks, list_copy_data, list_fields_to_copy, list_fields_to_postpone
        ):
            ...
            create_values = {
                ...
                "child_ids": [
                    ...
                    for vals in self._create_next_occurrences_values(
                        {child: recurrence for child in task.child_ids}
                    )
                ],
            }
