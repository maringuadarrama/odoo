...
class ProjectTask(models.Model):
    ...
    def _read_group_stage_ids(self, stages, domain):
        ...
        if (
            ...
            and not self._context.get("subtask_action")
    ...
    is_closed = fields.Boolean(
        "Closed state", compute="_compute_is_closed", search="_search_is_closed"
    )
    ...
    date_deadline = fields.Datetime(
        string="Deadline", index=True, tracking=True, copy=False
    )
    ...
    allow_milestones = fields.Boolean(
        related="project_id.allow_milestones", export_string_translation=False
    )
    ...
    allow_task_dependencies = fields.Boolean(
        related="project_id.allow_task_dependencies", export_string_translation=False
    )
    # Tracking of this field is done in the write function
    ...
    repeat_until = fields.Date(
        string="End Date", compute="_compute_repeat", compute_sudo=True, readonly=False
    )
    ...
    link_preview_name = fields.Char(
        compute="_compute_link_preview_name", export_string_translation=False
    )
    ...
    def _search_is_closed(self, operator, value):
        ...
        domain = [("state", "in", searched_states)]
        return domain
    ...
    def _inverse_state(self):
        ...
        tasks = self.filtered(
            lambda task: task.state in CLOSED_STATES
            ...
            for user_id in user_ids:
                ...
                if not stage:
                    stages = (
                        ...
                        .sudo()
                        .with_context(
                            lang=user_id.partner_id.lang, default_project_ids=False
                        )
                        .create(
                            self.with_context(
    ...
    def _compute_subtask_count(self):
        if not any(self._ids):
            for task in self:
                task.subtask_count, task.closed_subtask_count = len(
                    task.child_ids
    ...
    def copy_data(self, default=None):
        ...
        if not_project_user:
            vals_list = [
                {k: v for k, v in vals.items() if k in self.SELF_READABLE_FIELDS}
    ...
    def default_get(self, default_fields):
        ...
        if project_id:
            ...
            if (
                "company_id" in default_fields
                and "default_project_id" not in self.env.context
            ):
                vals["company_id"] = project.sudo().company_id.id
        elif (
            "default_user_ids" not in self.env.context and "user_ids" in default_fields
        ):
    ...
    def _ensure_fields_are_accessible(
        ...
        if (
            fields
            and (not check_group_user or self.env.user._is_portal())
            and not self.env.su
        ):
            ...
            if unauthorized_fields:
                unauthorized_field_list = format_list(
                    self.env, list(unauthorized_fields)
                )
                if operation == "read":
                    error_message = _(
                        "You cannot read the following fields on tasks: %(field_list)s",
                        field_list=unauthorized_field_list,
                    )
                else:
                    error_message = _(
                        "You cannot write on the following fields on tasks: %(field_list)s",
                        field_list=unauthorized_field_list,
                    )
            ...
            vals_no_sudo.update(
                {
                    ...
                    and key[8:] in self.SELF_WRITABLE_FIELDS
                    and self._fields[key[8:]].type in ("one2many", "many2many")
                }
            )
    ...
    def _get_portal_sudo_context(self):
        return {
            ...
            in (
                field
                for field in self.SELF_WRITABLE_FIELDS
                if self._fields[field].type not in ("one2many", "many2many")
            )
        }
    ...
    def create(self, vals_list):
            ...
            if vals.get("user_ids"):
                ...
                if not (vals.get("parent_id") or project_id):
                    user_ids = self._fields["user_ids"].convert_to_cache(
                        vals.get("user_ids", []), self.env["project.task"]
                    )
        ...
        if is_portal_user:
            ...
            self_no_sudo, self = self, self.sudo().with_context(
                self._get_portal_sudo_context()
            )
        tasks = super(
            ProjectTask, self.with_context(mail_create_nosubscribe=True)
        ).create(vals_list)
        if is_portal_user:
            ...
            for task in self:
                ...
                if project_link:
                    body = _(
                        ...
                        destination_project=self.project_id._get_html_link(
                            title=self.project_id.display_name
                        ),
                    )
                else:
    ...
    def _search_on_comodel(self, domain, field, comodel, additional_domain=None):
        ...
        filtered_domain = filter_domain_leaf(
            domain,
            lambda field_to_check: field_to_check
            in [
                field,
                f"{field}.id",
                f"{field}.name",
    ...
    def _send_email_notify_to_cc(self, partners_to_notify):
        ...
        for partner in partners_to_notify:
            ...
            assignation_msg = self.env["ir.qweb"]._render(
                "project.task_invitation_follower", values, minimal_qcontext=True
            )
            self.message_notify(
    ...
    def _ensure_personal_stages(self):
        ...
        if not ProjectTaskTypeSudo.search_count([("user_id", "=", user.id)], limit=1):
            ProjectTaskTypeSudo.with_context(
                lang=user.lang, default_project_id=False
            ).create(
                self.with_context(
                    lang=user.lang
    ...
    def message_new(self, msg, custom_values=None):
        ...
        if not msg.get("author_id") and msg.get("email_from"):
            author = self.env["mail.thread"]._partner_find_from_emails_single(
                [msg["email_from"]], no_create=False
            )
        ...
        partners = task._partner_find_from_emails_single(
            tools.email_split((msg.get("to") or "") + "," + (msg.get("cc") or "")),
            no_create=True,
        )
        ...
        if self.project_id:
            return {
                ...
                "views": [
                    (
                        self.env.ref(
                            "project.project_task_convert_to_subtask_view_form", False
                        ).id,
                        "form",
                    )
                ],
    ...
    def action_archive(self):
        ...
        self.filtered(
            lambda t: not t.display_in_project and t.parent_id
    ...
    def _get_thread_with_access(
        ...
        return super()._get_thread_with_access(
            thread_id, project_sharing_id=project_sharing_id, token=token, **kwargs
        )
