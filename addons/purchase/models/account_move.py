        ...
        if method in ("total_match", "po_match"):
            ...
            with self._get_edi_creation() as invoice:
                ...
                invoice.invoice_line_ids = [
                    Command.update(line.id, {"quantity": 0})
            ...
            with self._get_edi_creation() as invoice:
                ...
                invoice.invoice_line_ids = [
                    Command.update(
                        po_line.id, {"quantity": inv_line.quantity, "tax_ids": inv_line.tax_ids}
                    )
                    for po_line, inv_line in inv_and_po_lines
                ]
                invoice.invoice_line_ids = [
                    Command.delete(inv_line.id)
                    for _dummy, inv_line in inv_and_po_lines
                ]
