...
class AccountMove(models.Model):
    ...
    def write(self, vals):
        ...
        old_purchases = [
            move.mapped("line_ids.purchase_line_ids.order_id")
            for move in self.filtered(lambda m: m.line_ids.purchase_line_ids)
        ]
        res = super().write(vals)
        for i, move in enumerate(self.filtered(lambda m: m.line_ids.purchase_line_ids)):
    ...
    def _compute_count_purchase_order(self):
        for move in self:
            move.count_purchase_order = len(move.line_ids.purchase_line_ids.order_id)
    ...
    @api.onchange("purchase_bill_union_id", "purchase_order_id")
    def _onchange_purchase_auto_complete(self):
        ...
        * If it's a purchase order, "purchase_order_id" is set and this method will load lines.
        ...
        if self.purchase_bill_union_id.vendor_bill_id:
            ...
            self.purchase_order_id = self.purchase_bill_union_id.purchase_order_id
        self.purchase_bill_union_id = False
        if not self.purchase_order_id:
            return

        # Copy data from PO
        invoice_vals = self.purchase_order_id.with_company(
        ...
        # Copy purchase lines.
        po_lines = self.purchase_order_id.order_line_ids - self.invoice_line_ids.mapped(
            "purchase_line_ids"
        )
        ...
        if self.company_id != self.purchase_order_id.company_id:
    ...
    def _add_purchase_order_lines(self, purchase_order_lines):
        ...
        for po_line in purchase_order_lines:
            new_line_values = po_line._prepare_aml_from_pol_vals(self)
    ...
    def _find_matching_po_and_inv_lines(self, po_lines, inv_lines, timeout):
        ...
        purchase_lines = sorted(
            po_lines,
            key=lambda line: (line.price_unit, line.product_uom_qty - line.qty_invoiced),
            reverse=True,
        )
        matched_po_lines = []
        matched_inv_lines = []
            ...
            for invoice_line in invoice_lines:
                ...
                for purchase_line in purchase_lines:
                    ...
                    if (
                        ...
                        <= purchase_line.product_uom_qty - purchase_line.qty_invoiced
                    ):
    ...
    def _match_purchase_orders(
        ...
        if po_references and amount_total:
            ...
            if matching_purchase_orders:
                ...
                po_lines = [
                    line
                    for line in matching_purchase_orders.order_line_ids
                    if line.product_uom_qty
                ]
                po_lines_with_amount = [
                    {
                        "line": line,
                        "amount_to_invoice": (1 - line.qty_invoiced / line.product_uom_qty)
                        * line.price_total,
                    }
        ...
        if partner_id and amount_total:
            purchase_order_id_domain = common_domain + [
                ("partner_id", "child_of", [partner_id]),
                ("amount_total", ">=", amount_total - TOLERANCE),
                ("amount_total", "<=", amount_total + TOLERANCE),
            ]
            matching_purchase_orders = self.env["purchase.order"].search(
                purchase_order_id_domain
            )
        ...
        with self.env.cr.savepoint():
            with self._get_edi_creation() as invoice:
                ...
                for purchase_order in purchase_orders:
                    ...
                    invoice.purchase_order_id = purchase_order
    ...
    def _find_and_set_purchase_orders(
        ...
        if method in ("total_match", "po_match"):
            ...
            with self._get_edi_creation() as invoice:
                    ...
                    map(
                        lambda line: (
                            invoice.invoice_line_ids.filtered(
                                lambda l: l.purchase_line_ids
                                and line[0] in l.purchase_line_ids.ids
                            ),
                ...
                invoice.invoice_line_ids = [
                    Command.delete(inv_line.id) for _dummy, inv_line in inv_and_po_lines
                ]
    ...
    def _get_invoice_reference(self):
        ...
        vendor_refs = [
            ref
            for ref in set(
                self.invoice_line_ids.mapped("purchase_line_ids.order_id.partner_ref")
            )
            if ref
        ]
        if self.ref:
