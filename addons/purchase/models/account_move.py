...
class AccountMove(models.Model):
    ...
    def write(self, vals):
        old_purchases_dict = {
            move.id: move.mapped("line_ids.purchase_line_ids.order_id")
        ...
        for move in self.filtered(lambda m: m.line_ids.purchase_line_ids):
            ...
            old_purchases = old_purchases_dict.get(move.id, self.env["purchase.order"])

            diff_purchases = new_purchases - old_purchases
            if diff_purchases:
                ...
                po_lines_with_amount = [
                    {
                        "line": line,
                        "amount_to_invoice": (
                            1 - line.qty_invoiced / line.product_qty
                        )
                        * line.price_total,
                    }
