...
class PurchaseOrder(models.Model):
    ...
    tax_country_id = fields.Many2one(
        ...
        # Avoid access error on fiscal position, when reading a purchase order with 
        # company != user.company_ids
        compute_sudo=True,
        help='Technical field to filter the available taxes depending '
             'on the fiscal country and fiscal position.',
    )
    ...
    def onchange(self, values, field_names, fields_spec):
        ...
        if (
            any(
                self._must_delete_date_planned(field) for field in field_names
            )
            and 'value' in result
        ):
    ...
    def action_rfq_send(self):
        '''
        This function opens a window to compose an email, with the edi purchase template message
        loaded by default
        '''
        self.ensure_one()
    ...
    def _create_downpayments(self, line_vals):
        ...
        # a simple concatenation would cause all order_line to recompute,
    ...
    def action_merge(self):
        ...
        if all(len(rfq_bunch) == 1 for rfq_bunch in list(bunches_of_rfq_to_be_merge)):
            raise UserError(_(
                ...
                'destination, dropship address and agreement'
            ))
                ...
                for rfq_line in rfqs.order_line:
                    existing_line = oldest_rfq.order_line.filtered(
                        lambda l:
                            ...
                            and l.product_uom == rfq_line.product_uom
                            and l.product_packaging_id == rfq_line.product_packaging_id
                            and l.product_packaging_qty == rfq_line.product_packaging_qty
    ...
    def get_confirm_url(self, confirm_type=None):
        ...
        if confirm_type in ['reminder', 'reception', 'decline']:
            ...
            return self.get_portal_url(query_string=f'&{param}')
    ...
    def get_update_url(self):
        ...
        return self.get_portal_url(query_string=f'&{update_param}')
    ...
    def confirm_reminder_mail(self, confirmed_date=False):
        for order in self:
            if order.state in ['purchase', 'done'] and not order.mail_reminder_confirmed:
                ...
                order.message_post(
                    body=_(
                        '%(vendor)s confirmed the receipt will take place on %(date)s.',
    ...
    def _confirm_reception_mail(self):
        for order in self:
            if order.state in ['purchase', 'done'] and not order.mail_reception_confirmed:
                ...
                order.message_post(
                    body=_(
                        'The RFQ has been acknowledged by %s.',
                        order.partner_id.name
                    )
                )
        ...
        for order in self:
            if order.state in ['purchase', 'done'] and not order.mail_reception_declined:
                ...
                order.activity_schedule(
                    ...
                    note=_(
                        'The vendor asked to decline this confirmed RfQ, '
                        'if you agree on that, cancel this PO'
                    )
                )
                order.message_post(
                    body=_(
                        'The order receipt has been declined by %s.',
                        order.partner_id.name
                    )
                )
                ...
                order.message_post(
                    body=_(
                        'The RFQ has been declined by %s.',
                        order.partner_id.name
                    )
                )
    ...
    def _send_reminder_mail(self, send_single=False):
        ...
        if template:
            ...
            for order in orders:
                ...
                if date and (
                    send_single
                    or (date - relativedelta(days=order.reminder_date_before_receipt)).date() == datetime.today().date()
                ):
                    if send_single:
    ...
    def retrieve_dashboard(self):
        ...
        self.env.cr.execute(
            query,
            (one_week_ago, self.env.ref('purchase.mt_rfq_sent').id, self.env.company.id)
        )
