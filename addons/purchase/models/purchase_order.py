...
class PurchaseOrder(models.Model):
    ...
    date_approve = fields.Datetime(
        "Confirmation Date", readonly=True, index=True, copy=False
    )
    ...
    show_comparison = fields.Boolean(
        "Show Comparison", compute="_compute_show_comparison"
    )
    ...
    def _compute_show_comparison(self):
        ...
        for record in self:
            record.show_comparison = any(
                ...
                for p in record.order_line.product_id
    ...
    def _create_downpayments(self, line_vals):
        ...
        # create section
        if not any(
            line.display_type and line.is_downpayment for line in self.order_line
        ):
