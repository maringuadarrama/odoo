...
class PurchaseOrder(models.Model):
    ...
    product_id = fields.Many2one(
        related="order_line_ids.product_id",
        string="Product",
    )
    ...
    def copy(self, default=None):
        ...
        for line in new_orders.order_line_ids:
            if line.product_id:
    ...
    def _compute_user_id(self):
        for order in self:
            if order.partner_id and not (order._origin.id and order.purchase_user_id):
    ...
    @api.depends("order_line_ids", "order_line_ids.date_planned")
    ...
    def _compute_show_comparison(self):
        line_groupby_product = self.env["purchase.order.line"]._read_group(
            [
                ("product_id", "in", self.order_line_ids.product_id.ids),
                ("state", "=", "purchase"),
            ],
            ["product_id"],
            ["order_id:array_agg"],
        ...
        for order in self:
            ...
            if any(
                ...
                all(
                    ...
                    for line in order.order_line_ids.filtered(
        ...
        if operator == "=" and value or operator == "!=" and not value:
            purchase_lines_late = self.env["purchase.order.line"].search(
                [
                    ...
                    ("qty_transfered", "<", SQL("product_uom_qty")),
                ]
            )
    ...
    def print_quotation(self):
        self.filtered(lambda po: po.state == "draft").write({"printed_before": True})
    ...
    def _create_update_date_activity(self, updated_dates):
        ...
        activity = self.activity_schedule(
            ...
            user_id=self.purchase_user_id.id,
        )
    ...
    def _create_downpayments(self, line_vals):
        self.ensure_one()
        if not any(
            line.display_type and line.is_downpayment for line in self.order_line_ids
        ):
            section_line = self.order_line_ids.create(
                self._prepare_down_payment_section_values()
            )
        else:
            section_line = self.order_line_ids.filtered(
    ...
    def create_invoice(self):
        ...
        for order in self:
            ...
            invoice_vals = order._prepare_create_invoice_vals()
            for line in order.order_line_ids:
                ...
                if not float_is_zero(line.qty_to_invoice, precision_digits=precision):
                    if pending_section:
                        line_vals = pending_section._prepare_aml_from_pol_vals()
                        ...
                        sequence += 1
                        pending_section = None
                    line_vals = line._prepare_aml_from_pol_vals()
                    line_vals.update({"sequence": sequence})
            ...
            if oldest_rfq:
                ...
                for rfq_line in rfqs.order_line_ids:
                    ...
                    if len(existing_line) > 1:
                        existing_line[0].product_uom_qty += sum(
                            existing_line[1:].mapped("product_uom_qty")
                        )
    ...
    def _update_order_line_info(self, product_id, quantity, **kwargs):
        ...
        :rtype: float"""
        self.ensure_one()
        pol = self.order_line_ids.filtered(
            ...
            pol = self.env["purchase.order.line"].create(
                {
                    ...
                    "sequence": (
                        (self.order_line_ids and self.order_line_ids[-1].sequence + 1)
                        or 10
    ...
    def _get_action_add_from_catalog_extra_context(self):
        return {
            **super()._get_action_add_from_catalog_extra_context(),
            ...
            "product_catalog_digits": self.order_line_ids._fields[
                "price_unit"
            ].get_digits(self.env),
    ...
    def _get_orders_to_remind(self):
        ...
        return self.search(
            ...
            lambda p: p.mapped("order_line_ids.product_id.product_tmpl_id.type")
            != ["service"]
        )
    ...
    def get_order_timezone(self):
        ...
        self.ensure_one()
        return timezone(
            self.purchase_user_id.tz or self.company_id.partner_id.tz or "UTC"
        )
    ...
    def _prepare_create_invoice_vals(self):
        ...
        values = {
            ...
            "invoice_user_id": self.purchase_user_id.id,
    ...
    def prepare_dashboard(self):
        ...
        # easy counts
        groupby = ["priority", "purchase_user_id"]
        ...
        purchases = self.env["purchase.order"].search_fetch(
            ...
            ["create_date", "date_approve", "purchase_user_id"],
        )
        ...
        for po in purchases:
            ...
            if po.purchase_user_id == self.env.user:
    ...
    def _can_be_approved(self):
        ...
        # if self.company_id.po_double_validation == "two_step" and not is_manager:
        #     threshold = self.env.company.currency_id._convert(
        #         self.company_id.po_double_validation_amount,
        #         self.currency_id,
        #         self.company_id,
        #         self.date_order or fields.Date.today(),
        ...
        #         raise UserError(
        #             _(
        #                 "The total amount of the order is greater than %s. "
        #                 "An approval from a manager is required.",
        #                 self.company_id.po_double_validation_amount,
        #             )
        #         )
