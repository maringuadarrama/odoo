...
class res_partner(models.Model):
    ...
    property_purchase_currency_id = fields.Many2one(
        ...
        help='This currency will be used, instead of the default one, for purchases from '
             'the current partner',
    )
    ...
    receipt_reminder_email = fields.Boolean(
        'Receipt Reminder',
        company_dependent=True,
        help='Automatically send a confirmation email to the vendor X days before the expected '
             'receipt date, asking him to confirm the exact date.',
    )
