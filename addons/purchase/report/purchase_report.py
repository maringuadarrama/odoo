...
class PurchaseReport(models.Model):
    """Provides detailed purchase analysis and reporting.

    Handles purchase data, including order details, product information, and vendor data,
    to generate comprehensive purchase reports. It supports filtering and grouping by various fields such
    as product category, and customer country."""
    ...
    purchase_user_id = fields.Many2one("res.users", "Buyer", readonly=True)
    ...
    def _select(self) -> SQL:
        return SQL(
            ...
            o.dest_address_id,
            o.purchase_user_id AS purchase_user_id,
            o.date_order AS date_order,
            o.date_approve,
            EXTRACT(
                epoch from age(o.date_approve,o.date_order)) / (24*60*60)::decimal(16,2
            ) AS delay,
            EXTRACT(
                epoch from age(l.date_planned,o.date_order)) / (24*60*60)::decimal(16,2
            ) AS delay_pass,
            o.state,
            l.product_id,
            p.product_tmpl_id,
            ...
            SUM(
                l.product_uom_qty * line_uom.factor / product_uom.factor
            ) AS qty_ordered,
            SUM(
            ...
            CASE WHEN t.purchase_method = 'purchase'
                THEN SUM(l.product_uom_qty / line_uom.factor * product_uom.factor) - SUM(l.qty_invoiced / line_uom.factor * product_uom.factor)
            ...
            (
                SUM(l.product_uom_qty * l.price_unit / COALESCE(o.currency_rate, 1.0))
                / NULLIF(SUM(l.product_uom_qty / line_uom.factor * product_uom.factor), 0.0)
            )::decimal(16,2) * account_currency_table.rate AS price_average,
            SUM(
                l.price_subtotal / COALESCE(o.currency_rate, 1.0))::decimal(16,2
            ) * account_currency_table.rate AS untaxed_total,

            SUM(
                p.weight * l.product_uom_qty / line_uom.factor * product_uom.factor
            ) AS weight,
            SUM(
                p.volume * l.product_uom_qty / line_uom.factor * product_uom.factor
            ) AS volume,
            COUNT(*) AS nbr_lines
            """,
        )
