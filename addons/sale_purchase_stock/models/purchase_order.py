...
class PurchaseOrder(models.Model):
    ...
    @api.depends(
        "line_ids.move_dest_ids.group_id.sale_id",
        "line_ids.move_ids.move_dest_ids.group_id.sale_id"
    )
    ...
    def _get_sale_orders(self):
        linked_so = (
            self.line_ids.move_dest_ids.group_id.sale_id
            | self.env["stock.move"].browse(
                self.line_ids.move_ids._rollup_move_dests()
            ).group_id.sale_id
        )
        group_so = self.line_ids.procurement_group_id.sale_id
...
class PurchaseOrderLine(models.Model):
    ...
    def _prepare_stock_move_vals_list(self):
        res = super()._prepare_stock_move_vals_list()
        for re in res:
            ...
            if self.order_id.dest_address_id:
                # In a dropshipping context we do not need the description of the purchase order
                # or it will be displayed in Delivery slip report and it may be confusing for the
                # customer to see several times the same text (product name + description_picking).
    ...
    def _find_candidate(
        ...
        ) if values.get("sale_line_id") else self
    ...
    @api.model
    def _prepare_purchase_order_line_from_procurement(self, product_id, product_uom_qty, product_uom, location_dest_id, name, origin, company_id, values, po):
        res = super()._prepare_purchase_order_line_from_procurement(product_id, product_uom_qty, product_uom, location_dest_id, name, origin, company_id, values, po)
