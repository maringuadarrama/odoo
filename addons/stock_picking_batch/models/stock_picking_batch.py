...
class StockPickingBatch(models.Model):
    ...
    def _compute_move_ids(self):
        for batch in self:
            ...
            batch.show_check_availability = any(
                m.state not in ["assigned", "cancel", "done"] for m in batch.move_ids
            )
    ...
    def action_done(self):
        def has_no_quantity(picking):
            return all(
                not m.picked
                or float_is_zero(m.quantity, precision_rounding=m.product_uom.rounding)
            ...
            if not batch.picking_ids <= batch.allowed_picking_ids:
                ...
                raise UserError(
                    _(
                        ...
                        batch=batch.name,
                        incompatible_transfers=format_list(
                            self.env, erroneous_pickings.mapped("name")
                        ),
                    )
                )
