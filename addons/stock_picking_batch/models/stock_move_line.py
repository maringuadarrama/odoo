...
class StockMoveLine(models.Model):
    ...
    def _auto_wave(self):
        ...
        for line in self:
            ...
            # We want to find the most descendant location in the wave locations list that is
            # a parent of the line location. Since the wave locations are ordered by complete_name
            # (from the most descendant to the most ancestor), we can iterate in reverse order.
    ...
    def _auto_wave_lines_into_existing_waves(self, nearest_parent_locations=False):
        """Try to add move lines to existing waves if possible, return move lines of which
        no appropriate waves were found to link to
        :param nearest_parent_locations (defaultdict): the key is the move line and the value
        is the nearest parent location in the wave locations list"""
            ...
            if lines:
                ...
                if picking_type.batch_group_by_src_loc:
                    domain = expression.AND(
                        [
                            domain,
                            [("picking_ids.location_id", "in", lines.location_id.ids)],
                        ]
                    )
        ...
        for picking_type in picking_types:
            ...
            if picking_type.batch_group_by_partner:
                domain = expression.AND(
                    [
                        domain,
                        [("move_id.partner_id", "in", lines.move_id.partner_id.ids)],
                    ]
                )
            if picking_type.batch_group_by_destination:
                domain = expression.AND(
            ...
            if picking_type.batch_group_by_src_loc:
                domain = expression.AND(
                    [domain, [("location_id", "in", lines.location_id.ids)]]
                )
            if picking_type.batch_group_by_dest_loc:
                domain = expression.AND(
            ...
            if picking_type.wave_group_by_product:
                domain = expression.AND(
                    [domain, [("product_id", "in", lines.product_id.ids)]]
                )
            if picking_type.wave_group_by_category:
                domain = expression.AND(
                    [
                        domain,
                        [("product_id.categ_id", "in", lines.product_id.categ_id.ids)],
                    ]
                )
            if picking_type.wave_group_by_location:
                domain = expression.AND(
