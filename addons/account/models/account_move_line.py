...
class AccountMoveLine(models.Model):
    ...
    is_imported = (
        fields.Boolean()
    )  # Technical field indicating if the line was captured automatically by import/ocr etc
    ...
    _partner_id_ref_idx = models.Index(
        "(partner_id, ref)",
    )
    _date_name_id_idx = models.Index(
    ...
    _unreconciled_index = models.Index(
        "(account_id, partner_id) WHERE reconciled IS NOT TRUE AND parent_state = 'posted'",
    )
    _journal_id_neg_amnt_residual_idx = models.Index(
        "(journal_id) WHERE amount_residual < 0 AND parent_state = 'posted'",
    )
    # covers the standard index on account_id
    _account_id_date_idx = models.Index(
        "(account_id, date)",
    )
        ...
        for line in self.filtered(lambda l: l.move_id.inalterable_hash is False):
            if line.display_type == "payment_term":
                ...
                if (
                    n_terms > 1
                    or not line.name
                    or line._origin.name == line._origin.move_id.payment_reference
                    or (
                        line._origin.move_id.payment_reference
                        and line._origin.move_id.ref
                        and line._origin.name
                        == f"{line._origin.move_id.ref} - {line._origin.move_id.payment_reference}"
                    )
                ):
                    line.name = name
    ...
    def _compute_tax_tag_invert(self):
        for record in self:
            ...
            if not record.tax_repartition_line_id and not record.tax_ids:
                ...
                if (
                    record.display_type == "epd"
                ):  # In case of early payment, tax_tag_invert is independent of the balance of the line
    ...
    def _compute_tax_ids(self):
        for line in self:
            if (
                line.display_type in ("line_section", "line_note", "payment_term")
                or line.is_imported
            ):
                continue
    ...
    def create(self, vals_list):
        ...
        with moves._check_balanced(
            move_container
        ), ExitStack() as exit_stack, moves._sync_dynamic_lines(
            move_container
        ), self._sync_invoice(
            container
        ):
    ...
    def write(self, vals):
                        ...
                        ._read_group(
                            domain=[
                                (
                                    "matching_number",
                                    "in",
                                    [n for n in self.mapped("matching_number") if n],
                                )
                            ],
        ...
        with self.move_id._check_balanced(move_container), self.env.protecting(
    ...
    def _reconcile_plan_with_sync(self, plan_list, all_amls):
        ...
        for plan in plan_list:
            plan_results = self.with_context(
                ...
                or disable_partial_exchange_diff,
        ...
        if not self._context.get("move_reverse_cancel") and not self._context.get(
            "no_cash_basis"
        ):
            for plan in plan_list:
        ...
        if not self._context.get("no_exchange_difference"):
            for full_batch_index, full_batch in enumerate(full_batches):
                ...
                exchange_diff_values["to_post"] = all(
                    [aml.parent_state == "posted" for aml in exchange_lines_to_fix]
                )
        ...
        for full_batch_index, exchange_move in zip(
            exchange_diff_full_batch_index, exchange_moves
        ):
        ...
        for full_batch_index, full_batch in enumerate(full_batches):
            ...
            if full_batch["is_fully_reconciled"]:
                full_reconcile_values_list.append(
                    {
                        "exchange_move_id": full_batch.get("exchange_move")
                        and full_batch["exchange_move"].id,
    ...
    def _create_exchange_difference_moves(self, exchange_diff_values_list):
        ...
        self.with_context(no_exchange_difference=True)._reconcile_plan(
            reconciliation_plan
        )
