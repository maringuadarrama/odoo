...
class AccountMove(models.Model):
    ...
    def _compute_payment_state(self):
        ...
        for invoice in self:
            ...
            new_pmt_state = (
                "not_paid" if invoice.payment_state != "blocked" else "blocked"
            )
            if invoice.state == "posted" or (
                invoice.state == "draft" and not currency.is_zero(invoice.amount_total)
            ):
                ...
                if payment_state_matters:
                    ...
                    if currency.is_zero(invoice.amount_residual):
                        if any(
                            x["has_payment"] or x["has_st_line"]
                            for x in reconciliation_vals
                        ):
                            ...
                            in_reverse = invoice.move_type in (
                                "in_invoice",
                                "in_receipt",
                            ) and (
                        ...
                        and invoice.matched_payment_ids.filtered(
        ...
        if unbalanced_moves:
            ...
            for move_id, sum_debit, sum_credit in unbalanced_moves:
                ...
                error_msg += _(
                    ...
                    debit_total=format_amount(
                        self.env, sum_debit, move.company_id.currency_id
                    ),
                    credit_total=format_amount(
                        self.env, sum_credit, move.company_id.currency_id
                    ),
    ...
    def _can_be_unlinked(self):
        ...
        is_part_of_audit_trail = (
            self.posted_before and self.company_id.check_account_audit_trail
        )
        ...
        return (
            ...
            and not is_part_of_audit_trail
