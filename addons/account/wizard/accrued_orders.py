...
class AccountAccruedOrdersWizard(models.TransientModel):
    ...
    def _compute_move_vals(self):
        ...
        total_balance = 0.0
        for order in orders:
            product_lines = order.order_line_ids.filtered(lambda x: x.product_id)
            if len(orders) == 1 and product_lines and self.amount and order.order_line_ids:
                ...
                order.order_line_ids.invalidate_model(fnames)
