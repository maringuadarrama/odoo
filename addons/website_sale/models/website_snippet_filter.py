...
class WebsiteSnippetFilter(models.Model):
        ...
        if sale_orders:
            if self.env.context.get('hide_variants'):
                sold_products = Counter(
                    ...
                    for sol in sale_orders.order_line_ids                )
            else:
                sold_products = Counter(sol.product_id for sol in sale_orders.order_line_ids)
            if sold_products:
                domain = expression.AND([
                    domain,
    ...
    def _get_products_latest_viewed(self, website, limit, domain, **kwargs):
        ...
        if visitor:
            excluded_products = request.cart.order_line_ids.product_id.ids
    ...
    def _get_products_recently_sold_with(
        ...
        if current_template:
            ...
            if sale_orders:
                cart_products = request.cart.order_line_ids.product_id
                ...
                included_products = sale_orders.order_line_ids.product_id
                if self.env.context.get('hide_variants'):
