...
class SaleOrder(models.Model):
    ...
    def copy(self, default=None):
        ...
        reward_lines = new_orders.order_line_ids.filtered('is_reward_line')
        if reward_lines:
            reward_lines.unlink()
        return new_orders
    ...
    def action_confirm(self):
        for order in self:
            all_coupons = order.applied_coupon_ids | order.coupon_point_ids.coupon_id | order.order_line_ids.coupon_id
        ...
        reward_coupons = self.order_line_ids.coupon_id
    ...
    def action_cancel(self):
        ...
        # Remove any rewards
        self.order_line_ids.filtered(lambda l: l.is_reward_line).unlink()
    ...
    def _get_applied_global_discount_lines(self):
        ...
        """
        self.ensure_one()
        return self.order_line_ids.filtered(lambda l: l.reward_id.is_global_discount)
    ...
    def _get_reward_values_product(self, reward, coupon, product=None, **kwargs):
        ...
        return [{
            ...
            'sequence': max(self.order_line_ids.filtered(lambda x: not x.is_reward_line).mapped('sequence'), default=10) + 1,
    ...
    def _discountable_amount(self, rewards_to_ignore):
        ...
        for line in self.order_line_ids - self._get_no_effect_on_threshold_lines():
    ...
    def _discountable_order(self, reward):
        ...
        if reward.program_id.is_payment_program:
            ...
            lines = self.order_line_ids - self._get_no_effect_on_threshold_lines()
    ...
    def _discountable_specific(self, reward):
        ...
        order_lines = self.order_line_ids - self._get_no_effect_on_threshold_lines()
        ...
        order_lines -= self.order_line_ids.filtered('reward_id')
        cheapest_line = False
    ...
    def _get_reward_values_discount(self, reward, coupon, **kwargs):
        ...
        sequence = max(
            self.order_line_ids.filtered(lambda x: not x.is_reward_line).mapped('sequence'),
            default=10
        ) + 1
        ...
        if not discountable:
            if not reward_program.is_payment_program and any(line.reward_id.program_id.is_payment_program for line in self.order_line_ids):
                return [{
                    **base_reward_line_values,
    ...
    def _get_reward_programs(self):
        ...
        """
        self.ensure_one()
        return self.order_line_ids.reward_id.program_id
    ...
    def _recompute_prices(self):
        ...
        if any(line.is_reward_line for line in self.order_line_ids):
            self._update_programs_and_rewards()
    ...
    def _get_real_points_for_coupon(self, coupon, post_confirm=False):
        ...
        points -= sum(self.order_line_ids.filtered(lambda l: l.coupon_id == coupon).mapped('points_cost'))
    ...
    def _write_vals_from_reward_vals(self, reward_vals, old_lines, delete=True):
        """
        Update, create new reward line and delete old lines in one write on `order_line_ids`
        ...
        self.write({'order_line_ids': command_list})
    ...
    def _get_claimable_rewards(self, forced_coupons=None):
        ...
        all_coupons = forced_coupons or (self.coupon_point_ids.coupon_id | self.order_line_ids.coupon_id | self.applied_coupon_ids)
        if not all_coupons:
            return result

        has_payment_reward = any(line.reward_id.program_id.is_payment_program for line in self.order_line_ids)
        ...
        for coupon in all_coupons:
            ...
            for reward in coupon.program_id.reward_ids:
                ...
                if is_discount and not is_payment_program and reward in self.order_line_ids.reward_id:
                    continue
    ...
    def _update_programs_and_rewards(self):
        ...
        for program in points_programs:
            ...
            if 'error' in status:
                ...
                program_reward_lines = self.order_line_ids.filtered(lambda l: l.coupon_id in coupons_from_order)
        ...
        for program in coupon_programs:
            if program not in domain_matching_programs or\
                ...
                program_reward_lines = self.order_line_ids.filtered(lambda l: l.coupon_id in applied_coupon_per_program[program])
                program_reward_lines._reset_loyalty(True)
                lines_to_unlink |= program_reward_lines
        ...
        reward_line_pool = self.order_line_ids.filtered(lambda l: l.reward_id and l.coupon_id)._reset_loyalty()
        seen_rewards = set()
        line_rewards = []
        ...
        if order_line_update:
            self.write({'order_line_ids': order_line_update})
    ...
    def _get_not_rewarded_order_lines(self):
        return self.order_line_ids.filtered(lambda line: line.product_id and not line.reward_id)
    ...
    def _program_check_compute_points(self, programs):
        ...
        # Prepare amounts
        so_products_per_rule = programs._get_valid_products(self.order_line_ids.product_id)
    ...
    def action_confirm(self, send_email=False):
        ...
        :return: None
        """
        super().action_confirm(send_email=True)
