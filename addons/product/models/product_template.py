...
class ProductTemplate(models.Model):
    ...
    service_tracking = fields.Selection(
        ...
        required=True,
        default='no',
        compute='_compute_service_tracking', store=True,
        readonly=False,
    )
    ...
    uom_name = fields.Char(
        related='uom_id.name', string='Unit of Measure Name', readonly=True,
    )
    uom_category_id = fields.Many2one(
        related='uom_id.category_id', string='UoM Category',
    )
    ...
    volume = fields.Float(
        string='Volume',
        digits='Volume',
        compute='_compute_volume', store=True,
        inverse='_set_volume',
    )
    ...
    purchase_ok = fields.Boolean(
        string='Purchase',
        default=True,
        compute='_compute_purchase_ok', store=True,
        readonly=False,
    )
    ...
    def create(self, vals_list):
        '''
        Store the initial standard price in order to be able to retrieve the cost
        of a product template for a given date
        '''
    ...
    def write(self, vals):
        ...
        if (
            self._context.get('create_product_product', True) and 'attribute_line_ids' in vals
    ...
    def _create_variant_ids(self):
                    ...
                    if combination in existing_variants:
                        ...
                        if len(current_variants_to_create) > int(variant_limit):
                            raise UserError(_(
                                ...
                                'You should either not generate variants for each combination '
                                'or generate them on demand from the sales order. '
                                'To do so, open the form view of attributes and change the mode '
                                'of *Create Variants*.'
                            ))
        ...
        if variants_to_unlink:
            ...
            if self.exists() != self:
                raise UserError(_(
                    'This configuration of product attributes, values, and exclusions would '
                    'lead to no possible variant. Please archive or delete your product '
    ...
    def _is_combination_possible_by_config(self, combination, ignore_no_variant=False):
        '''
        Return whether the given combination is possible according to the config
        of attributes on the template
        ...
        :return: wether the given combination is possible according to the config
        of attributes on the template
        :rtype: bool
        '''
        self.ensure_one()
        ...
        return isinstance(
            next(
                self._filter_combinations_impossible_by_config([combination],ignore_no_variant),
                False
            ),
            models.BaseModel
        )
    ...
    def _get_variant_id_for_combination(self, filtered_combination):
        ...
        return self.env['product.product'].sudo().with_context(
            active_test=False
        ).search(domain, order='active DESC', limit=1).id
