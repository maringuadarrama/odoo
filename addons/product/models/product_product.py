...
class ProductProduct(models.Model):
    ...
    def init(self):
        ...
        '''
        self.env.cr.execute(
            'CREATE UNIQUE INDEX IF NOT EXISTS product_product_combination_unique ON '
            '%s (product_tmpl_id, combination_indices) WHERE active is true'
            % self._table
        )
    ...
    def _check_duplicated_product_barcodes(self, barcodes_within_company, company_id):
        ...
        products_by_barcode = self.sudo().read_group(
            domain,
            ['barcode', 'id:array_agg'],
            ['barcode']
        )
        duplicates_as_str = '\n'.join(
            _(
                ...
                product_list=format_list(
                    self.env,
                    [p.display_name for p in self.search([('id', 'in', record['id'])])]
                ),
            )
    ...
    def unlink(self):
        ...
        for product in existing_products:
            ...
            has_other_products = (
                product_ids_by_template_id.get(product.product_tmpl_id.id, set()) - {product.id}
            )
    ...
    def name_search(self, name='', args=None, operator='ilike', limit=100):
        ...
        if not products:
            if is_positive:
                ...
                if limit_rest is None or limit_rest > 0:
                    products |= self.search_fetch(
                        expression.AND([domain, [('id', 'not in', products.ids)],
                        [('name', operator, name)]]),
                        ['display_name'],
                        limit=limit_rest
                    )
                ...
                products = self.search_fetch(
                    expression.AND([domain, domain_neg]),
        ...
        if (
            ...
            products = self.search_fetch(
                expression.AND([domain, match_domain]),
                ['display_name'], limit=limit
            )
        if not products and (partner_id := self.env.context.get('partner_id')):
            # still no results, partner in context: search on supplier info  
            # as last hope to find something
            supplier_domain = [
                ('partner_id', '=', partner_id),
                '|',
            ...
            for supplier_info in product.seller_ids:
                if supplier_info.partner_id.id == product._context.get('partner_id'):
                    ...
                    product.partner_ref = '%s%s' % (
                        product.code and '[%s] ' % product.code or '', product_name
                    )
                    break
    ...
    def _compute_write_date(self):
        ...
        for record in self:
            record.write_date = max(
                record.write_date or self.env.cr.now(), record.product_tmpl_id.write_date
            )
    ...
    def _compute_can_image_variant_1024_be_zoomed(self):
        for record in self:
            record.can_image_variant_1024_be_zoomed = (
                ...
                and is_image_size_above(record.image_variant_1920, record.image_variant_1024)
            )
    ...
    def get_product_multiline_description_sale(self):
        ...
        (do not use for purchases or other display reasons that don't intend to use
        'description_sale').
        It will often be used as the default description of a sale order line referencing
        this product.
        '''
    ...
    def toggle_active(self):
        ...
        (and vice versa, unarchiving the related product template if there is now an active
        product.product)
        '''
