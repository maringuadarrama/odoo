...
class ProductComboItem(models.Model):
    ...
    company_id = fields.Many2one(
        related="combo_id.company_id", precompute=True, store=True
    )
    combo_id = fields.Many2one(
        comodel_name="product.combo", ondelete="cascade", required=True, index=True
    )
    ...
    currency_id = fields.Many2one(
        comodel_name="res.currency", related="product_id.currency_id"
    )
    ...
    extra_price = fields.Float(
        string="Extra Price", digits="Product Price", default=0.0
    )
