...
class ProductTemplateAttributeValue(models.Model):
    ...
    html_color = fields.Char(
        related='product_attribute_value_id.html_color', string='HTML Color Index'
    )
    ...
    exclude_for = fields.One2many(
        ...
        string='Exclude for',
        help='Make this attribute value not compatible with other values of the '
             'product or some attribute values of optional and accessory products.',
    )
    ...
    def create(self, vals_list):
        if any('ptav_product_variant_ids' in v for v in vals_list):
            ...
            raise UserError(_(
                'You cannot update related variants from the values. '
                'Please update related values from the variants.'
            ))
    ...
    def write(self, vals):
        ...
        if pav_in_values or product_in_values:
            for ptav in self:
                if pav_in_values and ptav.product_attribute_value_id.id != vals['product_attribute_value_id']:
                    raise UserError(_(
                        'You cannot change the value of the value %(value)s '
                        'set on product %(product)s.',
                        value=ptav.display_name,
                        product=ptav.product_tmpl_id.display_name,
                    ))

                if product_in_values and ptav.product_tmpl_id.id != vals['product_tmpl_id']:
                    raise UserError(_(
                        'You cannot change the product of the value %(value)s '
                        'set on product %(product)s.',
                        value=ptav.display_name,
                        product=ptav.product_tmpl_id.display_name,
                    ))
