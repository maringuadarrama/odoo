...
class PaymentTransaction(models.Model):
    ...
    count_sale_order_ids = fields.Integer(
        string="# of Sales Orders",
        compute="_compute_count_sale_order_ids",
    )
    ...
    def _compute_count_sale_order_ids(self):
        for trans in self:
            trans.count_sale_order_ids = len(trans.sale_order_ids)
    ...
    def _post_process(self):
        ...
        super(
            ...
            self.filtered(
                lambda tx:
                    tx.state not in ["pending", "authorized", "done"]
            )
        )._post_process()
