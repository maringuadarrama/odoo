...
class AccountMove(models.Model):
    """Extends the 'account.move' model to integrate sales-related functionalities and UTM tracking.

    This module enhances the account move (invoice) model by linking it to sales orders, tracking UTM parameters,
    and managing downpayments. It also provides tools for computing sales team assignments, handling invoice payments,
    and reconciling transactions with sales orders."""
    ...
    def action_post(self):
        ...
        # We cannot change lines content on locked SO,
        # changes on invoices are not forwarded to the SO if the SO is locked
    ...
    def _post(self, soft=True):
        ...
        # It's useful when you have a "paid" sale order (using a payment
        # transaction) and you invoice it later.
    ...
    def _action_invoice_ready_to_be_sent(self):
        # Make sure the send invoice CRON is called when
        # an invoice becomes ready to be sent by mail.
    ...
    def _get_partner_credit_warning_exclude_amount(self):
        ...
        # (see _compute_credit_to_invoice and _compute_amount_to_invoice_taxinc from 'sale.order' )
        exclude_amount = super()._get_partner_credit_warning_exclude_amount()
        for order in self.line_ids.sale_line_ids.order_id:
            order_amount = min(
                self._get_sale_order_invoiced_amount(order), order.amount_to_invoice_taxinc
            )
