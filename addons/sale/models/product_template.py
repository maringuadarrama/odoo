...
class ProductTemplate(models.Model):
    ...
    service_type = fields.Selection(
        ...
        help='Manually set quantities on order: Invoice based on the manually entered quantity, '
             ...
             'Timesheets on contract: Invoice based on the tracked hours on the '
             'related timesheet.\n'
             'Create a task and track hours: Create a task on the sales order validation and '
    ...
    expense_policy = fields.Selection(
        ...
        readonly=False,
        help='Validated expenses, vendor bills, or stock pickings (set up to track costs) '
            ...
            if len(incompatible_fields) > 1:
                raise ValidationError(_(
                    ...
                    value_list=format_list(
                        self.env,
                        [field_descriptions[v] for v in incompatible_fields]
                    ),
                ))
    ...
    def _onchange_type(self):
        ...
        if self._origin and self.sales_count > 0:
            res['warning'] = {
                'title': _('Warning'),
                'message': _(
                    'You cannot change the product\'s type because it is already used in '
                    'sales orders.'
                )
            }
        return res
