...
class ProductTemplate(models.Model):
    ...
    expense_policy = fields.Selection(
        ...
        readonly=False,
        help="Validated expenses, vendor bills, or stock pickings "
             "(set up to track costs) can be invoiced to the customer at either cost or sales price."
    )
    ...
    def _compute_count_sales(self):
        for product in self:
            product.count_sales = float_round(sum([p.count_sales for p in product.with_context(active_test=False).product_variant_ids]), precision_rounding=product.uom_id.rounding)
