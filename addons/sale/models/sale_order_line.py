...
class SaleOrderLine(models.Model):
    ...
    def write(self, vals):
        ...
        if "product_id" in vals and any(
            sol.product_id.id != vals["product_id"] and not sol.product_updatable
            for sol in self
        ):
    ...
    @api.depends(
        "order_id.pricelist_id", "product_id", "product_uom_id", "product_uom_qty"
    )
    ...
    def _compute_price_unit(self):
        for line in self.filtered(lambda l: not l.display_type):
            # Don't compute the price for deleted lines or
            # if the price has been manually set or there is already invoiced amount.
    ...
    def _compute_discount(self):
        ...
        for line in self.filtered(lambda l: not l.display_type):
            ...
            if (
                ...
                or not line.pricelist_item_id._show_discount()
            ):
    ...
    def _compute_amounts(self):
        for line in self.filtered(lambda l: not l.display_type):
            ...
            line.price_unit_discounted_taxexc = base_line[
                "price_unit_discounted_excluded_currency"
            ]
            line.price_unit_discounted_taxinc = base_line[
