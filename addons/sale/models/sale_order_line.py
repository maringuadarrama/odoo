...
class SaleOrderLine(models.Model):
    ...
    company_id = fields.Many2one(
        related="order_id.company_id", store=True, precompute=True,
        index=True,
    )
    ...
    currency_id = fields.Many2one(
        related="order_id.currency_id", store=True, precompute=True,
        depends=["order_id.currency_id"],
    )
    order_partner_id = fields.Many2one(
        related="order_id.partner_id", store=True, precompute=True,
        string="Customer",
        index=True,
    )
    salesman_id = fields.Many2one(
        related="order_id.user_id", store=True, precompute=True,
        string="Salesperson",
    )
    state = fields.Selection(
        related="order_id.state", store=True, precompute=True,
        string="Order Status",
        copy=False,
    )
    ...
    product_custom_attribute_value_ids = fields.One2many(
        ...
        compute="_compute_product_custom_attribute_value_ids", store=True, precompute=True,
        readonly=False,
        copy=True
    )
    ...
    product_no_variant_attribute_value_ids = fields.Many2many(
        ...
        string="Extra Values",
        compute="_compute_product_no_variant_attribute_value_ids", store=True, precompute=True,
        readonly=False,
        ondelete="restrict",
    )
    ...
    product_uom_qty = fields.Float(
        ...
        required=True,
        default=1.0,
        compute="_compute_product_uom_qty", store=True, precompute=True,
        readonly=False,
    )
    name = fields.Text(
        string="Description",
        required=True,
        compute="_compute_name", store=True, precompute=True,
        readonly=False,
    )
    ...
    price_unit = fields.Float(
        ...
        required=True,
        compute="_compute_price_unit", store=True, precompute=True,
        readonly=False,
    )
    ...
    discount = fields.Float(
        string="Discount (%)",
        digits="Discount",
        compute="_compute_discount", store=True, precompute=True,
        readonly=False,
    )
    tax_ids = fields.Many2many(
        comodel_name="account.tax",
        string="Taxes",
        compute="_compute_tax_ids", store=True, precompute=True,
        readonly=False,
        check_company=True,
    ...
    customer_lead = fields.Float(
        string="Lead Time",
        required=True,
        compute="_compute_customer_lead", store=True, precompute=True,
        readonly=False,
    ...
    def write(self, vals):
        ...
        if any(self.order_id.mapped("locked")) and any(f in vals.keys() for f in protected_fields):
    ...
    @api.depends("order_id", "order_partner_id", "product_id")
    ...
    @api.depends("product_id")
    def _compute_product_custom_attribute_value_ids(self):
        for line in self:
            ...
            valid_values = line.product_id.product_tmpl_id.valid_product_template_attribute_line_ids.\
    ...
    @api.depends("product_id")
    def _compute_product_no_variant_attribute_value_ids(self):
        for line in self:
            if not line.product_id:
    ...
    def _compute_qty_delivered_method(self):
        """Sale module compute delivered qty for product
        [("type", "in", ["consu"]), ("service_type", "=", "manual")]
        -consu + expense_policy : analytic (sum of analytic unit_amount)
        -consu + no expense_policy : manual (set manually on SOL)
        -service (+ service_type="manual", the only available option) : manual
        This is true when only sale is installed:
        sale_stock redifine the behavior for "consu" type,
    ...
    def _compute_qty_delivered(self):
        """This method compute the delivered quantity of the SO lines: it covers
        the case provide by sale module, aka expense/vendor bills
        (sum of unit_amount of AAL), and manual case. This method should be overridden
        to provide other way to automatically compute delivered qty. Overrides should
        take their concerned so lines, compute and set the `qty_delivered` field,
        and call super with the remaining records."""
    ...
    def _compute_qty_invoiced(self):
        ...
        intentional: if a refund made would automatically decrease the invoiced quantity,
    ...
    def _compute_qty_invoiced_posted(self):
        ...
        The only difference lies in the fact that for accounting purposes,
        we only want the quantities of the posted invoices.
        We need a dedicated computation because the triggers are different and could
        lead to incorrect values for "qty_invoiced" when computed together."""
        for line in self:
    ...
    @api.depends("state", "product_uom_qty", "qty_delivered", "qty_invoiced")
    ...
    @api.depends("state", "product_uom_qty", "qty_delivered", "qty_invoiced", "qty_to_invoice")
                ...
                if (
                    ...
                    if invoice_line.move_id.move_type == "out_invoice":
                        amount_invoiced += invoice_line.currency_id._convert(
                            invoice_line.price_subtotal, line.currency_id, line.company_id, invoice_date
                        )
                    elif invoice_line.move_id.move_type == "out_refund":
                        amount_invoiced -= invoice_line.currency_id._convert(
                            invoice_line.price_subtotal, line.currency_id, line.company_id, invoice_date
    ...
    @api.depends(
        "state", "product_id", "product_uom_qty", "price_unit",
        "qty_delivered", "untaxed_amount_invoiced"
    )
        ...
        for line in self:
            ...
            if line.state == "sale":
                ...
                if len(line.tax_ids.filtered(lambda tax: tax.price_include)) > 0:
                    ...
                    # `compute_all()` method has to be called to retrieve the subtotal without them.
                    # `price_reduce_taxexcl` cannot be used as it is computed from `price_subtotal` field.
                    # (see upper Note)
                ...
                if any(inv_lines.mapped(lambda l: l.discount != line.discount)):
                    ...
                    for l in inv_lines:
                        if len(l.tax_ids.filtered(lambda tax: tax.price_include)) > 0:
                            amount += l.tax_ids.compute_all(l.currency_id._convert(
                                l.price_unit,
                                line.currency_id,
                                line.company_id,
                                l.date or fields.Date.today(),
                                round=False
                            ) * l.quantity)["total_excluded"]
                        else:
    ...
    @api.depends("product_uom_qty", "discount", "price_total", "qty_delivered", "qty_invoiced_posted")
    ...
    def _get_sale_order_line_multiline_description_sale(self):
        ...
        e.g:
        -custom attributes and attributes that don't create variants, both introduced by the
        "product configurator"
        -in event_sale we need to know specifically the sales order line as well as the product to generate
        the name: the product is not sufficient because we also need to know the event_id and the event_ticket_id
        (both which belong to the sale order line)."""
        self.ensure_one()
        description = (
    ...
    def _get_sellable_lines_domain(self):
    ...
    def _prepare_procurement_values(self, group_id=False):
        """Prepare specific key for moves or other components that will be created from a
        stock rulecoming from a sale order line. This method could be override in order to
        add other custom key that could be used in move/po creation."""
        return {}
