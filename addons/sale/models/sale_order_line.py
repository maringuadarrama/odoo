...
class SaleOrderLine(models.Model):
    ...
    product_custom_attribute_value_ids = fields.One2many(
        ...
        compute='_compute_custom_attribute_values', store=True, precompute=True,
        readonly=False,
        copy=True,
    )
    ...
    product_no_variant_attribute_value_ids = fields.Many2many(
        ...
        string='Extra Values',
        compute='_compute_no_variant_attribute_values', store=True, precompute=True,
        readonly=False,
        ondelete='restrict',
    )
    ...
    tax_id = fields.Many2many(
        ...
        string='Taxes',
        check_company=True,
        compute='_compute_tax_id', store=True, precompute=True,
        readonly=False,
    ...
    def write(self, values):
        if (
            'display_type' in values
            and self.filtered(lambda line: line.display_type != values.get('display_type'))
        ):
            raise UserError(_(
                'You cannot change the type of a sale order line. '
                'Instead you should delete the current line and '
        ...
        # Don't recompute the package_id if we are setting the quantity of the items 
    ...
    def _get_sale_order_line_multiline_description_sale(self):
        ...
        information that only exists on the sale order line itself.
        e.g:
        - custom attributes and attributes that don't create variants, both introduced by the
        'product configurator'
        - in event_sale we need to know specifically the sales order line as well as the product
        to generate the name: the product is not sufficient because we also need to know
        the event_id and the event_ticket_id (both which belong to the sale order line).
        '''
        self.ensure_one()
        description = (
    ...
    def _compute_product_uom_qty(self):
        for line in self:
            ...
            product_uom_qty = packaging_uom._compute_quantity(
                line.product_packaging_qty * qty_per_packaging,
                line.product_uom
            )
            if float_compare(
                product_uom_qty,
        ...
        for line in self:
            ...
            if line.product_id and line.product_uom_qty and line.product_uom:
                suggested_packaging = line.product_id.packaging_ids.filtered(
                    lambda p: p.sales and (p.product_id.company_id <= p.company_id <= line.company_id)
    ...
    def _get_delivered_quantity_by_analytic(self, additional_domain):
        ...
        for uom, so_line, unit_amount_sum, move_line_id_count_distinct, count in data:
            ...
            # avoid counting unit_amount twice when dealing with multiple analytic lines
            # on the same move line
    ...
    def _compute_qty_invoiced_posted(self):
        '''
        This method is almost identical to '_compute_qty_invoiced()'. The only difference lies
        in the fact that for accounting purposes, we only want the quantities of the posted invoices.
        We need a dedicated computation because the triggers are different and could lead
        to incorrect values for 'qty_invoiced' when computed together.
        '''
        for line in self:
    ...
    @api.depends(
        'state', 'product_id', 'untaxed_amount_invoiced', 'qty_delivered', 'product_uom_qty',
        'price_unit'
    )
                    ...
                    for l in inv_lines:
                        if len(l.tax_ids.filtered(lambda tax: tax.price_include)) > 0:
                            amount += l.tax_ids.compute_all(l.currency_id._convert(
                                l.price_unit,
                                line.currency_id,
                                line.company_id, l.date or fields.Date.today(),
    ...
    @api.depends('state', 'product_id', 'qty_invoiced', 'qty_delivered')
    ...
    def _prepare_procurement_values(self, group_id=False):
        ...
        coming from a sale order line. This method could be override in order to add other custom
    ...
    def _get_product_catalog_lines_data(self, **kwargs):
        ...
        Note: This method cannot be called with multiple records
        that have different products linked.
