...
class AccountMoveLine(models.Model):
    ...
    def _prepare_analytic_lines(self):
        ...
        if move_to_reinvoice.filtered(lambda aml: not aml.move_id.reversed_entry_id):
    ...
    def _sale_create_reinvoice_sale_line(self):
        ...
        #     the index of the sale line to create in the `sale_line_values_to_create` (not existing
        #     case, which will happen more often than the first one).
        map_move_sale_line = {}
        for move_line in self:
            ...
            if (
                ...
                if sale_line:  # already search, so reuse it. sale_line can be sale.order.line record or index of a 'to create values' in `sale_line_values_to_create`
                    ...
                    # should be create, so use the index of creation values instead of browse record
                    else:
    ...
    def _sale_get_invoice_price(self, order):
        '''
        Based on the current move line, compute the price to reinvoice the analytic line that is
        going to be created (so the price of the sale line).
        '''
        self.ensure_one()
