...
class AccountMoveLine(models.Model):
    ...
    def _prepare_analytic_lines(self):
        ...
        if move_to_reinvoice.filtered(
            lambda aml:
                not aml.move_id.reversed_entry_id
                and aml.product_id
        ):
    ...
    def _sale_can_be_reinvoice(self):
        ...
        For Vendor Bill flow, if the product has a 'erinvoice policy' and is a cost,
        then we will find the SO on which reinvoice the AAL
        '''
        self.ensure_one()
        ...
        return (
            ...
            and self.product_id.expense_policy not in [False, 'no']
        )

    def _sale_create_reinvoice_sale_line(self):
        sale_order_map = self._sale_determine_order()
        sale_line_values_to_create = []  # the list of creation values of sale line to create.
        # in the sales_price-delivery case, we can reuse the same sale line.
        # This cache will avoid doing a search each time the case happen
        ...
        for move_line in self:
            ...
            if sale_order.state in ('draft', 'sent'):
                raise UserError(_(
                    'The Sales Order %(order)s to be reinvoiced must be validated '
            ...
            if (
                ...
                # already search, so reuse it. sale_line can be sale.order.line record
                # or index of a 'to create values' in `sale_line_values_to_create`
                if sale_line: 
        ...
        for move_line_id, unknown_sale_line in map_move_sale_line.items():
            ...
            elif isinstance(unknown_sale_line, models.BaseModel): 
                result[move_line_id] = unknown_sale_line
        return result

    def _sale_determine_order(self):
        '''
        Get the mapping of move.line with the sale.order record on which its analytic entries
        should be reinvoiced
    ...
    def _sale_get_invoice_price(self, order):
        '''
        Based on the current move line, compute the price to reinvoice the analytic line
        that is going to be created (so the price of the sale line).
        '''
        self.ensure_one()
