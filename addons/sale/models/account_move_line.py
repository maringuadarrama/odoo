...
class AccountMoveLine(models.Model):
    ...
    def _get_downpayment_lines(self):
        ...
        return self.sale_line_ids.filtered(
            "is_downpayment"
        ).invoice_lines.filtered(lambda line: line.move_id._is_downpayment())

    def _prepare_analytic_lines(self):
        """Note: This method is called only on the move.line that having an analytic distribution,
        and so that should create analytic entries."""
        values_list = super()._prepare_analytic_lines()
        # filter the move lines that can be reinvoiced:
        # a cost (negative amount) analytic line without SO line
        # but with a product can be reinvoiced
