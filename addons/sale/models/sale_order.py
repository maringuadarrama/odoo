...
class SaleOrder(models.Model):
    ...
    expected_date = fields.Datetime(
        ...
        help='Delivery date you can promise to the customer, computed from the minimum lead time '
             'of the order lines.'
    )
    ...
    note = fields.Html(
        ...
        compute='_compute_note', store=True, precompute=True,
        readonly=False,
    )
    ...
    is_expired = fields.Boolean(
        'Is Expired', compute='_compute_is_expired',
    )
    ...
    @api.depends('state', 'date_order', 'order_line.customer_lead')
    def _compute_expected_date(self):
        '''
        For service and consumable, we only take the min dates. This method is extended in
        sale_stock to take the picking_policy of SO into account.
        '''
    ...
    @api.depends('partner_id', 'date_order', 'client_order_ref', 'origin',)
    ...
    def _get_confirmation_template(self):
        ...
        default_confirmation_template = (
            ...
            and self.env['mail.template'].browse(
                int(default_confirmation_template_id)
            ).exists()
        )
    ...
    def _prepare_down_payment_section_line(self, **optional_values):
        ...
        :param dict optional_values: any parameter that should be added to the returned
    ...
    def message_post(self, **kwargs):
        if self.env.context.get('mark_so_as_sent'):
            self.filtered(
                lambda o:
                    o.state == 'draft').with_context(tracking_disable=True).write({'state': 'sent'}
            )
    ...
    def _get_name_tax_totals_view(self):
        '''
        This method can be inherited by localizations who want to localize the taxes displayed 
        on the portal and sale order report.
        '''
