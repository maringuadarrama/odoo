...
class SaleOrder(models.Model):
    ...
    def _search_invoice_ids(self, operator, value):
        if operator == "in" and value:
            ...
            order_ids = self._search(
                [
                    (
                        "order_line_ids.invoice_lines.move_id.move_type",
                        "in",
    ...
    def _compute_amount_to_invoice(self):
        for order in self:
            order.amount_to_invoice = sum(
                order.order_line_ids.mapped("amount_to_invoice")
            )
    ...
    def action_quotation_send(self):
        ...
        ).order_line_ids._validate_analytic_distribution()
    ...
    def action_confirm(self):
        ...
        for order in self:
            error_msg = order._prepare_confirmation_error_message()
            if error_msg:
    ...
    def _recompute_taxes(self):
        lines_to_recompute = self.order_line_ids.filtered(
    ...
    def _prepare_confirmation_error_message(self):
