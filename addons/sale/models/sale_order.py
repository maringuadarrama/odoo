...
from odoo.tools import SQL, format_amount, is_html_empty
...
class SaleOrder(models.Model):
    ...
    def _compute_team_id(self):
        cached_teams = {}
        for order in self:
            default_team_id = order._get_default_team_id()
    ...
    def _compute_duplicated_order_ids(self):
        order_to_duplicate_orders = self._get_duplicated_orders()
        for order in self:
        ...
        for order in self:
            ...
            if not order.company_id:
                raise ValidationError(
                    _(
                        "The company is required, please select one before "
                        "making any other changes to the sale order."
                    )
                )
    ...
    def action_confirm(self):
        ...
        for order in self:
            error_msg = order._get_confirmation_error_message()
            if error_msg:
    ...
    def _create_downpayment_invoices(self):
    ...
    def _get_confirmation_error_message(self):
    ...
    def _get_duplicated_orders(self):
