...
class SaleOrder(models.Model):
    ...
    _inherit = [
        ...
        'portal.mixin', 'product.catalog.mixin',
        'utm.mixin',
    ]
    ...
    fiscal_position_id = fields.Many2one(
        ...
        readonly=False,
        check_company=True,
        help='Fiscal positions are used to adapt taxes and accounts '
             'for particular customers or sales orders/invoices.'
    ...
    tax_country_id = fields.Many2one(
        ...
        # Avoid access error on fiscal position when reading a sale order
        # with company != user.company_ids
        compute_sudo=True,
    )
    ...
    transaction_ids = fields.Many2many(
        ...
        column1='sale_order_id', column2='transaction_id',
        string='Transactions',
        readonly=True,
        copy=False,
    )
    ...
    @api.depends('date_order', 'partner_id', 'client_order_ref', 'origin')
    ...
    def _fetch_duplicate_orders(self):
        ...
        result = self.env.execute_query(SQL(
            ...
            SELECT
              ...
              FROM
                sale_order
                JOIN sale_order AS duplicate_order ON sale_order.company_id = duplicate_order.company_id
    ...
    def _notify_by_email_prepare_rendering_context(
        ...
        subtitles = [
            f'{record.name} - {record.partner_id.name}' if record.partner_id else record.name
        ]
