...
from odoo.tools import format_amount, format_date, is_html_empty
...
class SaleOrder(models.Model):
    ...
    def _compute_has_archived_products(self):
        for order in self:
            order.has_archived_products = any(
                not product.active for product in order.order_line_ids.product_id
            )

    @api.depends(
        "company_id", "currency_id", "payment_term_id", "order_line_ids.price_subtotal"
    )
    ...
    @api.depends("state", "date_order", "order_line_ids.customer_lead")
    def _compute_date_planned(self):
        ...
        for order in self:
            ...
            if dates_list:
                order.date_planned = order._select_date_planned(dates_list)
            else:
                order.date_planned = False
    ...
    def _compute_invoice_ids(self):
        ...
        for order in self:
            invoices = order.order_line_ids.invoice_line_ids.move_id.filtered(
    ...
    def _compute_invoice_state(self):
        ...
        for order in confirmed_orders:
            ...
            if any(invoice_state == "to do" for invoice_state in line_invoice_state):
    ...
    def _search_invoice_ids(self, operator, value):
        if operator == "in" and value:
            ...
            #
            # We cannot just search [('order_line_ids.invoice_line_ids', '=', False)]
            ...
            #
            # Domain below returns subset of ('order_line_ids.invoice_line_ids', '!=', False)
            order_ids = self._search(
                [
                    (
                        "order_line_ids.invoice_line_ids.move_id.move_type",
                        "in",
        ...
        return [
            ...
            ("order_line_ids.invoice_line_ids.move_id", operator, value),
        ]
    ...
    def _add_base_lines_for_early_payment_discount(self):
        ...
        if (
            ...
            for line in self.order_line_ids.filtered(lambda x: not x.display_type):
    ...
    def _create_invoices(self, grouped=False, final=False, date=None):
        ...
        for order in self:
            ...
            for line in invoiceable_lines:
                ...
                invoice_line_vals.append(
                    Command.create(
                        line._prepare_aml_from_sol_vals(sequence=invoice_item_sequence)
                    ),
                )
                invoice_item_sequence += 1
    ...
    def _create_invoices_downpayment(self):
    ...
    def _select_date_planned(self, date_planneds):
        self.ensure_one()
    ...
    def _get_product_documents(self):
        self.ensure_one()
        documents = (
            self.order_line_ids.product_id.product_document_ids
            | self.order_line_ids.product_template_id.product_document_ids
        )
    ...
    def _can_cancel_proper_state(self):
        self.ensure_one()
