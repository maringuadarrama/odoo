...
class SaleOrder(models.Model):
    ...
    def _notify_by_email_prepare_rendering_context(
        self, message, msg_vals=False, model_description=False, force_email_company=False, force_email_lang=False
    ):
    ...
    def _compute_field_value(self, field):
        ...
        filtered_self = self.filtered(
            lambda so: so.ids and (so.user_id or so.partner_id.user_id) and so._origin.invoice_status != "upselling"
        )
    ...
    def _compute_partner_shipping_id(self):
        for order in self:
            order.partner_shipping_id = (
                order.partner_id.address_get(["delivery"])["delivery"] if order.partner_id else False
            )
    ...
    def _compute_invoice_status(self):
        ...
        line_invoice_status_all = [
            ...
            for order, invoice_status in self.env["sale.order.line"]._read_group(
                lines_domain + [("order_id", "in", confirmed_orders.ids)], ["order_id", "invoice_status"]
            )
        ]
