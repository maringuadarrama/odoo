...
class SaleAdvancePaymentInv(models.TransientModel):
    """Wizard to manage the creation of advance invoices for sales orders.

    This wizard allows users to generate invoices based on down payments or
    percentages of the total sales order amount. It supports different payment
    methods, enabling flexible billing workflows according to business needs."""
    ...
    def _compute_has_down_payments(self):
        for wizard in self:
            wizard.has_down_payments = bool(
                wizard.sale_order_ids.order_line_ids.filtered("is_downpayment")
            )
    ...
    def _compute_invoice_amounts(self):
        for wizard in self:
            wizard.amount_invoiced = sum(
                wizard.sale_order_ids._origin.mapped("amount_invoiced_taxinc")
            )
    ...
    def _create_invoices(self, sale_orders):
        ...
        if self.advance_payment_method == "delivered":
            ...
            if self.advance_payment_method == "fixed":
                delta_amount = (invoice.amount_total - self.fixed_amount) * (
                    1 if invoice.is_inbound() else -1
                )
                if not order.currency_id.is_zero(delta_amount):
                    ...
                    if product_lines and tax_lines and receivable_line:
                        ...
                        for lines, attr, sign in (
                            ...
                            for line in lines:
                                if (
                                    order.currency_id.compare_amounts(remaining, 0)
                                    != delta_sign
                                ):
                                    break
                                amt = delta_sign * max(
    ...
    def _prepare_down_payment_section_values(self, order):
        return {
            ...
            "is_downpayment": True,
            "sequence": order.order_line_ids
            and order.order_line_ids[-1].sequence + 1
            or 10,
        }
    ...
    def _prepare_down_payment_lines_values(self, order):
        ...
        for line in order_lines:
            ...
            account = product_account.get("downpayment") or product_account.get(
                "income"
            )
            ...
            for fixed_tax in fixed_taxes:
                ...
                if fixed_tax.include_base_amount:
                    pct_tax = taxes[list(taxes).index(fixed_tax) + 1 :].filtered(
                        lambda t: t.is_base_affected and t.amount_type != "fixed"
                    )
                else:
        ...
        for (
            ...
            downpayment_line_map.setdefault(
                grouping_key,
                {
    ...
    def _prepare_invoice_values(self, order, so_lines, accounts):
        self.ensure_one()
        return {
            **order._prepare_create_invoice_vals(),
            "invoice_line_ids": [
                Command.create(
                    line._prepare_aml_from_sol_vals(
