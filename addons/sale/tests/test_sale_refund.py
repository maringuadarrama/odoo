...
class TestSaleRefund(TestSaleCommon):
    ...
    def test_refund_create(self):
        ...
        for line in self.sale_order.order_line:
            if line.product_id.invoice_policy == "delivery":
                ...
                if line == self.sol_prod_order:
                    self.assertEqual(
                        line.qty_to_invoice, 0.0, "The ordered sale line are totally invoiced (qty to invoice is zero)"
                    )
                    self.assertEqual(
                        line.qty_invoiced,
                        5.0,
        ...
        self.assertEqual(
            self.sale_order.count_invoice, 2, "The SO should have 2 related invoices: the original, the new refund"
        )
        self.assertEqual(
        ...
        for line in self.sale_order.order_line:
            if line.product_id.invoice_policy == "delivery":
                ...
                if line == self.sol_prod_order:
                    ...
                    self.assertEqual(
                        line.untaxed_amount_to_invoice, 0.0, "Amount to invoice is zero as the refund is not validated"
                    )
                    self.assertEqual(
                        line.untaxed_amount_invoiced,
                        line.price_unit * 5,
        ...
        for line in self.sale_order.order_line:
            if line.product_id.invoice_policy == "delivery":
                ...
                if line == self.sol_prod_order:
                    ...
                    self.assertEqual(
                        line.untaxed_amount_invoiced, 0.0, "Amount invoiced decreased as the refund is now confirmed"
                    )
                    self.assertEqual(
                        len(line.invoice_lines),
                        2,
    ...
    def test_refund_modify(self):
        ...
        # Check invoice's type and number
        self.assertEqual(
            invoice_refund.move_type, "out_invoice", "The last created invoiced should be a customer invoice"
        )
        self.assertEqual(invoice_refund.state, "draft", "Last Customer invoices should be in draft")
        self.assertEqual(
            self.sale_order.count_invoice,
            3,
    ...
    def test_refund_invoice_with_downpayment(self):
        ...
        credit_note_wizard = (
            self.env["account.move.reversal"]
            .with_context({"active_ids": [so_invoice.id], "active_id": so_invoice.id, "active_model": "account.move"})
            .create(
                {
        ...
        self.assertEqual(
            sol_product.untaxed_amount_invoiced, 0.0, "Amount invoiced decreased as the refund is now confirmed"
        )
        self.assertEqual(
