...
class TestSaleOrder(SaleCommon):
    ...
    def test_validity_days(self):
        ...
        sale_order = self._create_sale_order()
        self.assertFalse(
            sale_order.validity_date, "No validity date must be specified if the company validity duration is 0"
        )
    ...
    def test_so_names(self):
        ...
        self.assertIn(
            self.sale_order.partner_id.name, self.sale_order.with_context(sale_show_partner_name=True).display_name
        )
    ...
    def test_so_discount_is_not_reset(self):
        ...
        with patch("odoo.addons.sale.models.sale_order_line.SaleOrderLine" "._compute_discount") as patched:
            self.sale_order.action_confirm()
    ...
    def test_price_recomputation_on_readonly_unit_price(self):
        ...
        self.sale_order.write(
            {
                "order_line": [Command.update(product_sol.id, {"product_uom_qty": 4.0, "technical_price_unit": 22.0})],
            }
        )
...
class TestSaleOrderInvoicing(AccountTestInvoicingCommon, SaleCommon):
    def test_invoice_state_when_ordered_quantity_is_negative(self):
        ...
        self.assertTrue(
            sale_order.invoice_status == "invoiced", 'Sale: The invoicing status of the SO should be "invoiced"'
        )
...
class TestSalesTeam(SaleCommon):
    ...
    def test_assign_sales_team_when_changing_user(self):
        ...
        sale_order = self.env["sale.order"].create(
            {"user_id": self.user_not_in_team.id, "partner_id": self.partner.id, "team_id": self.sale_team_2.id}
        )
    ...
    def test_keep_sales_team_when_changing_user_with_no_team(self):
        ...
        sale_order = self.env["sale.order"].create({"partner_id": self.partner.id, "team_id": self.sale_team_2.id})
    ...
    def test_sale_order_analytic_distribution_change(self):
        ...
        self.assertFalse(
            sol_no_analytic_account.analytic_distribution, "The compute should not overwrite what the user has set."
        )
    ...
    def test_cannot_assign_tax_of_mismatch_company(self):
        ...
        sale_order = self.env["sale.order"].create({"partner_id": self.partner.id, "company_id": company_a.id})
