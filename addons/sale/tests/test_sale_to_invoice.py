...
class TestSaleToInvoice(TestSaleCommon):
    ...
    def test_downpayment_line_name(self):
        ...
        dp_line = sale_order.order_line.filtered(lambda sol: sol.is_downpayment and not sol.display_type)
    ...
    def test_downpayment_percentage_tax_icl(self):
        ...
        self.assertEqual(
            downpayment_line.price_unit, self.sale_order.amount_total / 2, "downpayment should have the correct amount"
        )
        ...
        self.assertEqual(
            downpayment_aml.price_total, self.sale_order.amount_total / 2, "downpayment should have the correct amount"
        )
        self.assertEqual(
            downpayment_aml.price_unit, self.sale_order.amount_total / 2, "downpayment should have the correct amount"
        )
        invoice.action_post()
        self.assertEqual(
            downpayment_line.price_unit, self.sale_order.amount_total / 2, "downpayment should have the correct amount"
    ...
    def test_invoice(self):
        ...
        for line in self.sale_order.order_line:
            if line.product_id.invoice_policy == "delivery":
                ...
                self.assertEqual(
                    line.qty_invoiced, 0.0, "Invoiced quantity should be zero as no any invoice created for SO"
                )
                self.assertEqual(
                ...
                self.assertEqual(
                    line.qty_to_invoice, line.product_uom_qty, "Quantity to invoice should be same as ordered quantity"
                )
                self.assertEqual(
                    line.qty_invoiced, 0.0, "Invoiced quantity should be zero as no any invoice created for SO"
                )
                ...
                self.assertEqual(
                    line.untaxed_amount_invoiced, 0.0, "The invoiced amount should be zero, as the line is confirmed"
                )
        ...
        for line in self.sale_order.order_line:
            if line.product_id.invoice_policy == "delivery":
                ...
                self.assertEqual(
                    line.qty_invoiced, 0.0, "Invoiced quantity should be zero as delivered lines are not delivered yet"
                )
                self.assertEqual(
                    line.untaxed_amount_to_invoice,
                    0.0,
        ...
        for line in self.sale_order.order_line:
            if line.product_id.invoice_policy == "delivery":
                ...
                if line == self.sol_prod_order:
                    self.assertEqual(
                        line.qty_to_invoice, 2.0, "The ordered sale line are totally invoiced (qty to invoice is zero)"
                    )
                    self.assertEqual(
                        line.qty_invoiced,
                        3.0,
                    ...
                    self.assertEqual(
                        line.qty_to_invoice, 1.0, "The ordered sale line are totally invoiced (qty to invoice is zero)"
                    )
                    self.assertEqual(
                        line.qty_invoiced,
                        2.0,
    ...
    def test_multi_company_invoice(self):
        ...
        self.context.update(
            allowed_company_ids=[yet_another_company_id, self.env.company.id], company_id=yet_another_company_id
        )
    ...
    def test_invoice_analytic_rule_with_account_prefix(self):
        ...
        self.env["sale.order.line"].create({"order_id": so.id, "name": "test", "product_id": self.product_a.id})
    ...
    def test_group_invoice(self):
        ...
        self.assertEqual(
            len(res["domain"][0][2]),
            2,
            "Invoicing 3 orders for the same partner with 2 currencies" "should create exactly 2 invoices.",
        )
    ...
    def test_sale_order_standard_flow_with_invoicing(self):
        ...
        self.assertTrue(
            self.sale_order.invoice_status == "no", 'Sale: SO status after invoicing should be "nothing to invoice"'
        )
        ...
        self.assertTrue(
            self.sale_order.invoice_status == "to invoice", 'Sale: SO status after delivery should be "to invoice"'
        )
