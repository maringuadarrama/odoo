...
class TestSaleOrderDownPayment(TestSaleCommon):
    ...
    def test_tax_fixed_amount_price_include(self):
        tax_fix = self.create_tax(
            5, {"amount_type": "fixed", "include_base_amount": True, "price_include_override": "tax_included"}
        )
    ...
    def test_tax_price_include_negative_amount_rounding_final_invoice(self):
        ...
        downpayment = (
            self.env["sale.advance.payment.inv"]
            .with_context({**so_context, "raise_if_nothing_to_invoice": False})
    ...
    def test_so_downpayment_invoice_credited_reinvoiced(self):
        ...
        credit_note_wizard = (
            ...
            .with_context(
                {
                    "active_ids": [invoice_to_be_refund.id],
    ...
    def test_downpayment_description(self):
        ...
        action = invoicing_wizard.create_invoices()
        so_dp_line = sale_order.order_line.filtered(lambda sol: sol.is_downpayment and not sol.display_type)
