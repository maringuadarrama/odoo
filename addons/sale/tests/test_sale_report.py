...
class TestSaleReportCurrencyRate(SaleCommon):
    ...
    def test_sale_report_foreign_currency(self):
        ...
        currency_rates = (companies + self.env.company).mapped("currency_id")._get_rates(self.env.company, today)
        ...
        for company in companies:
            ...
            for date in (past_day, today):
                for pricelist in pricelists:
                    ...
                    expected_reported_amount += order.amount_total / order.currency_rate * conversion_rate
    ...
    def test_sale_report_with_downpayment(self):
        ...
        self.assertEqual(
            float_compare(
                amount_line["untaxed_amount_invoiced:sum"], 200, precision_rounding=order.currency_id.rounding
            ),
            0,
        )
        self.assertEqual(
