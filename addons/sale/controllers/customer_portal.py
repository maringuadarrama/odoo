...
class CustomerPortal(payment_portal.PaymentPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if "count_quotation" in counters:
            values["count_quotation"] = (
                SaleOrder.search_count(self._prepare_quotations_domain(partner)) if SaleOrder.has_access("read") else 0
            )
    ...
    def portal_order_page(
        self, order_id, report_type=None, access_token=None, message=False, download=False, downpayment=None, **kw
    ):
        try:
    ...
    def portal_quote_accept(self, order_id, access_token=None, name=None, signature=None):
        ...
        if not order_sudo._has_to_be_paid():
            order_sudo._action_confirm_and_send()
    ...
    def portal_quote_decline(self, order_id, access_token=None, decline_message=None, **kwargs):
        ...
        if order_sudo._has_to_be_signed() and decline_message:
            ...
            order_sudo.message_post(
                author_id=(
                    order_sudo.partner_id.id if request.env.user._is_public() else request.env.user.partner_id.id
                ),
