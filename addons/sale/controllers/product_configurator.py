...
class SaleProductConfiguratorController(Controller):
    """Controller for handling product configuration in the sales module.

    This module provides endpoints and methods to retrieve and compute product information,
    including prices, attributes, and optional products, for use in the product configurator.
    It supports dynamic pricing, attribute combinations, and optional product management."""
                    ...
                    attribute_values=[
                        dict(
                            **ptav.read(["name", "html_color", "image", "is_custom"])[0],
                            price_extra=self._get_ptav_price_extra(ptav, currency, so_date, product_or_template),
                        )
    ...
    def _get_basic_product_information(self, product_or_template, pricelist, combination, **kwargs):
        ...
        basic_information = dict(**product_or_template.read(["description_sale", "display_name"])[0])
        ...
        if not product_or_template.is_product_variant:
            ...
            if combination_name:
                basic_information.update(display_name=f"{basic_information['display_name']} ({combination_name})")
        price, pricelist_rule_id = request.env["product.template"]._get_configurator_display_price(
            product_or_template.with_context(**product_or_template._get_product_price_context(combination)),
            pricelist=pricelist,
            **kwargs,
        )
        return dict(
    ...
    def sale_product_configurator_get_values(
        ...
        **kwargs,
        ...
        if ptav_ids:
            ...
            unconfigured_ptals = (product_template.attribute_line_ids - combination.attribute_line_id).filtered(
                lambda ptal: ptal.attribute_id.display_type != "multi"
            )
            combination += unconfigured_ptals.mapped(lambda ptal: ptal.product_template_value_ids._only_active()[:1])
            ...
            optional_products=[
                dict(
                    **self._get_product_information(
                        ...
                        optional_product_template._get_first_possible_combination(parent_combination=combination),
                        currency,
                        pricelist,
                        so_date,
    ...
    def sale_product_configurator_get_optional_products(
        ...
        **kwargs,
        ...
        product_template = request.env["product.template"].browse(product_template_id)
        parent_combination = request.env["product.template.attribute.value"].browse(parent_ptav_ids + ptav_ids)
        currency = request.env["res.currency"].browse(currency_id)
        pricelist = request.env["product.pricelist"].browse(pricelist_id)
        return [
            dict(
                **self._get_product_information(
                    optional_product_template,
                    optional_product_template._get_first_possible_combination(parent_combination=parent_combination),
                    currency,
                    pricelist,
